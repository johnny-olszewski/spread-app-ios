...
| ID | Item | Description | Impact | Effort | Risk |
| --- | --- | --- | --- | --- | --- |
| TF-07 | Password visibility toggle | SecureFields on `LoginSheet`, `SignUpSheet`, and `SetNewPasswordSheet` have no way to reveal the typed password. Add a show/hide eye icon button to each `SecureField` so users can verify what they've entered. | Medium: common iOS convention; reduces frustration on auth forms. | Low: `@State private var isVisible` + conditional `TextField`/`SecureField` swap per field. | Low |
| TF-08 | Resend verification from sign-in error | When a user signs in and gets "Please verify your email first", there is no quick path to resend the verification email. They must back out, tap "Create Account", and re-enter their email. Add an inline "Resend verification email" button to the sign-in error section when the error is `emailNotConfirmed`. | Medium: prevents dead-end UX for users who lost their confirmation email. | Low: detect `emailNotConfirmed` error state in `LoginSheet`; show a secondary action button that calls `authManager.resendVerification`. | Low |
| TF-09 | Change password while signed in | `AuthManager.updatePassword` exists but is only reachable via the password-reset deeplink. Users who want to change their password while signed in have no in-app path — they must sign out and go through "Forgot Password". Add a "Change Password" section to `ProfileSheet` with current-password and new-password fields. | Medium: expected account-management capability; absence forces users off the happy path. | Low-Medium: new section in `ProfileSheet` using existing `updatePassword` method. No backend changes needed. | Low |
...
| AS-13 | Data deletion support | App Store guidelines require that apps with account creation also provide a way to delete the user's account and associated data. Implement a delete-account flow in profile settings that removes the Supabase auth record and all user data. | High: App Store requirement; rejection risk without it. | Medium: Supabase delete user API + cascade delete of all user data + local SwiftData clear. | Medium: data destruction path requires careful ordering to avoid partial deletes. |
| AS-14 | Terms of service and privacy policy | Add accessible links to Terms of Service and Privacy Policy in the sign-up flow and settings. Documents must exist at public URLs before App Store submission. | High: required by App Store guidelines and legally necessary for any data-collecting app. | Low: UI wiring only; legal document creation is out of scope for engineering. | Low |
