...
| --- | --- | --- | --- | --- | --- |
| TF-01 | Remove debug print statements | Nine `print()` calls exist in production view code (`TaskCreationSheet`, `NoteCreationSheet`, `SpreadCreationSheet`, `SpreadPickerView`, `EntryRowView`). These leak internal state and add noise in production. | High: debug output should never ship. | Low: find and remove or gate with `#if DEBUG`. | Low |
...
| TF-03 | Auth error surfacing | Login, sign-up, and forgot-password flows do not visibly show errors to the user (wrong password, email already exists, network failure). Users receive no feedback when auth operations fail. | High: silent failure is confusing and makes the app appear broken. | Low-Medium: add `.alert()` bindings to auth sheet view models. | Low |
| TF-04 | Auth loading states | Auth operations (login, sign-up, password reset) have no loading indicators. Buttons remain tappable during async requests, risking duplicate submissions. | High: unresponsive UI during async operations hurts perceived quality. | Low: add an `isLoading` flag and `ProgressView` overlay to each auth sheet. | Low |
...
| ID | Item | Description | Impact | Effort | Risk |
| --- | --- | --- | --- | --- | --- |
| TF-40 | Smoke test: auth flows | Add Swift Testing tests for login success, login failure (wrong password), sign-up success, and forgot-password submission. Use mock auth service. | High: auth is a gate for all product value; regression here is critical. | Low-Medium | Low |
