...
- Main spread task lists keep transparent task rows over a solid list backing so the spread dot-grid remains visible. For open tasks, tapping anywhere on the row activates inline editing and focuses the title field without changing the row's overall layout height. Completed or cancelled task rows still open the full edit sheet on tap. [SPRD-124, SPRD-132, SPRD-142]
...
- While an open task row is in inline edit mode, the leading status icon becomes the reusable task status toggle aligned with the title row, and a secondary action row appears underneath with only two actions: a pencil-writing button that commits any inline title draft and opens the full edit sheet, and a right-arrow migration `Menu`. [SPRD-142]
