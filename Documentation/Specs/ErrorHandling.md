...
- **Sync errors**: Sync failures are non-blocking. Automatic retry occurs with exponential backoff (2s base, 300s max). A non-tappable error banner appears below the navigator strip with text "Last sync failed · Pull down to retry"; it clears on next successful sync. [SPRD-85, SPRD-134, SPRD-135] — **Superseded by SPRD-305** (`ReleaseHardening.md`): sync/offline status becomes app-wide (not Spreads-tab-only), a queued mutation that fails to serialize is quarantined rather than silently dropped, and Settings gains a Sync detail surface with a manual retry.
- **Network errors**: When offline, the app continues to function normally with local data. When connectivity returns, sync resumes automatically. Offline state is surfaced in the pull-to-refresh indicator only ("Offline"); no persistent banner is shown. [SPRD-85, SPRD-134, SPRD-135] — **Superseded by SPRD-305** (`ReleaseHardening.md`): a consistent offline indicator is shown app-wide.
- **App initialization errors**: If the SwiftData container fails to create on launch, the app shows a fatal error screen with a message to restart the app. No recovery is attempted. [SPRD-TBD] — **Superseded by SPRD-303** (`ReleaseHardening.md`): initialization failure no longer calls `fatalError`; a readable error screen offers an in-place **Try Again** that re-runs initialization.
