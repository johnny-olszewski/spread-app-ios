...
- Remaining for this task: rename `ChangeAwareTaskRepository`/`ChangeAwareNoteRepository` (and `SwiftData`/`Test`-prefixed variants) to drop the now-unnecessary `ChangeAware` qualifier, per `[2/n]`'s and SPRD-245's original renaming plan.
