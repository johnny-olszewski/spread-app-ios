...
### [SPRD-254] Refactor: Split current assignment from migration history on Task/Note - [ ] In Progress
...
- Remaining for this task: the two AC-mandated dedicated tests (entry-count-independent `spreadKeys` perf instrumentation; migration-history display-unchanged assertions) — the model/logic work itself is done and regression-proven, these are additional coverage on top.
