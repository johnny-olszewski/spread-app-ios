...
- **Progress (commits landed on feature/SESH-24)**:
  ...
  - Remaining for this task: remove `hasPreferredAssignment`/`storedHasPreferredAssignment`, swap all ~80 call sites to `date == nil`/`date != nil`, rework the Task creation/edit UI toggle; collapse `SyncEntityType`; rewrite `SyncSerializer` and `SyncEngine`'s pull/apply dispatch for the new unified tables (this will also fix the `SyncDurabilityIntegrationTests` failures above); apply the SQL migration to `spread-prod`; final full test suite + build verification.
