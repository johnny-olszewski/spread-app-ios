...
### [SPRD-249] Refactor: Additive incremental dictionary-keyed index and JournalManager-equivalent facade - [ ] In Progress
...
- Remaining for this task: mutation primitives (`upsertTask`/`removeTask`/etc. for all four entity kinds, using `repatchDataModel(for:)` which already exists but is unused until this lands) (`[5/n]`), and the remaining test categories (single-mutation-no-full-rebuild instrumentation via `fullLoadCount`, O(1)-lookup-under-large-N).
