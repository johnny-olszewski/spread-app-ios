...
- **Acceptance Criteria**:
  ...
  - [ ] All ~24 existing direct `.font(.body)`/`.title`/`.title2`/`.title3`/`.headline`/`.caption`/etc. call sites across the app are migrated to the corresponding `SpreadTheme.Typography` member, with no visual/size/weight regression.
  - [ ] The "always use `SpreadTheme.Typography`, never a raw system text style" convention is documented (`CLAUDE.md` and/or `DesignSystem.md`).
  - [x] Project builds with no errors or warnings.
- **Tests**:
...
- Remaining for this task: migrate the ~24 direct/raw font call sites onto `SpreadTheme.Typography`; document the convention; manual visual verification.
