...
- Remaining for this task: `.inFlight`/`.inbox` collection cases, `ReviewPanelView` segmented control (reusing `EntrySheetChoiceRow`), always-visible `listBullets` toggle, auto-close removal, empty states, inbox row-tap → `TaskEntrySheet` (ACs 1–7).
