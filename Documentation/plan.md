...
- **Acceptance Criteria**:
  - [ ] For each `SyncEntityType` with one or more pending mutations, `SyncEngine.push()` issues exactly one RPC call carrying all of that type's pending mutations, instead of one RPC call per mutation.
  - [ ] All 8 `merge_X_batch` Postgres functions exist in `baseline_schema.sql`, each accepting a jsonb array of the same per-row fields the scalar function takes, with per-row `auth.uid()` authorization (no batch-level trust boundary).
  - [ ] A malformed/failing row inside an otherwise-successful batch call does not abort the rest of that batch — other rows in the same call still succeed, and the function's returned per-row result array reflects per-row success/failure.
  - [ ] A whole-call failure (e.g. transport error) increments `retryCount` for every mutation in that batch and leaves them all in the outbox, matching today's per-mutation failure/backoff behavior exactly (no finer-grained handling here).
  - [ ] No local entity data is reverted or lost in either failure mode — local SwiftData writes remain final and independent of network/sync success, confirmed by the new test below.
  - [ ] Mutations that fail `buildMergeParams` (malformed `recordData`) are still filtered out and deleted from the outbox before the batch is built, unchanged from today's per-mutation behavior.
  - [ ] `Task.checkCancellation()` is preserved between entity-type batches (not per-row within a batch).
  ...
  - [ ] The updated `baseline_schema.sql` is applied to `spread-dev` first via `mcp__supabase__apply_migration`, verified with `list_tables`/`execute_sql` smoke queries, then promoted to `spread-prod` after confirmation — per the remote-migration-caution memory, confirm real row counts before applying to either project.
  - [ ] Project builds with no errors or warnings; full existing test suite passes unmodified.
- **Tests**:
  - [ ] Unit test asserting N pending mutations of the same entity type produce exactly 1 RPC call (not N), via a mocked `mergeRPCCaller` returning a batch-result array.
  - [ ] Unit test for partial-failure handling: some rows in a batch succeed and some fail; only the failed mutations keep `retryCount` incremented and remain in the outbox, succeeded ones are deleted.
  - [ ] Unit test simulating a failing batch RPC call end-to-end: local entity data is unchanged, the outbox mutation is retained with `retryCount` incremented (not deleted), and a subsequent successful retry clears it.
  - [ ] Existing `SyncEngineTests` suite passes with the mocked caller updated to the new batch-call shape.
...
- **Progress (commits landed on feature/SESH-26)**
  ...
  - Remaining for this task: rework `SyncEngine.push()` to batch per entity type with updated `MergeRPCCaller` signature and partial-failure handling; remove unused scalar `merge_X` functions; apply schema to spread-prod (spread-dev skipped — decommissioned) with row-count/smoke-query verification.