...
### [SPRD-255] Refactor: Additive TaskCoordinator/NoteCoordinator consolidating mutation + migration coordination - [ ] In Progress
...
- Remaining for this task: wire `JournalManager`'s 18 task/note CRUD + migration methods to delegate to the new coordinators (replacing their inline bodies), removing the `TODO: [SPRD-255]` markers, and confirm `JournalManager`'s own existing black-box test suites still pass unmodified.
