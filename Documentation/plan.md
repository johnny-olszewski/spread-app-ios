...
### [SPRD-255] Refactor: Additive TaskCoordinator/NoteCoordinator consolidating mutation + migration coordination - [x] Done
...
- **Acceptance Criteria**:
  ...
  - [x] New concrete `NoteCoordinator` added, covering all of `JournalManager`'s current note CRUD + migration methods (`addNote`, `updateNoteTitle`/`updateNoteMetadata`/`updateNoteDateAndPeriod`, `deleteNote`, `migrateNote`), depending only on `NoteRepository` and `JournalRuleEngine`.
  - [x] Neither type declares or conforms to a new protocol; no "Standard" naming.
  - [x] `JournalManager`'s public method signatures for these 18 methods are unchanged (views call them identically); each body becomes a thin delegation to the new coordinator plus the `spreads`-lookup/`upsertTask`/`upsertNote` patching the coordinator itself doesn't do.
  - [x] Project builds with no errors or warnings; full existing test suite passes unmodified in intent (this is a refactor, not a behavior change).
- **Tests**:
  ...
  - [x] Confirm `JournalManager`'s own existing test suites still pass unmodified after the delegation — proving the extraction is behavior-preserving.
...
- **Implementation notes** (2026-06-26): considered making this generic (`EntryCoordinator<E: AssignableEntry>`) per the user's request to prioritize genericity where reasonable. Concluded a single generic type covering *all* CRUD methods isn't a good fit — `addTask`/`addNote` have different field sets (priority/dueDate/body vs. content), `updateTaskMetadata` touches 5 LWW-tracked fields Note doesn't have, and `updateTaskStatus`/`clearTaskPreferredAssignment`/batch migration have no Note equivalent — forcing these into one generic type would mean dead optional parameters or a field-bag abstraction, exactly what CLAUDE.md's "don't force aggressive generic `Entry` abstractions" rule warns against. The migration *mechanics* inside `migrateTask`/`moveTask`/`migrateTasksBatch`/`migrateNote` *are* identical except for the terminal status value, which is the "shared helper where the business rule is truly identical" case CLAUDE.md does sanction — factored into `JournalRuleEngine.migrateAssignment<E: AssignableEntry & AnyObject>` (`[1/n]`), used by both coordinators' migration methods instead of three near-duplicate inline blocks.
- **Progress (commits landed on feature/SESH-24)**:
  1. `[SPRD-255][1/n]` — Added `JournalRuleEngine.migrateAssignment<E: AssignableEntry & AnyObject>`, the shared generic migration mechanic (see Implementation notes above). Required an `AnyObject` constraint plus an internal `var entry = entry` rebind — `AssignableEntry`'s `{ get set }` requirements don't get writable access through a generic `let` parameter even with `AnyObject` alone, confirmed empirically. Purely additive — not yet called anywhere (flagged with a `TODO: [SPRD-255]` pointing at this task's next increment). Added 3 new tests (new-destination creation, historical-destination revival preserving `Assignment.id`, nil-source/Inbox-origin case). Verified via `-only-testing:SpreadTests/JournalRuleEngineTests` (23/23 pass) and a full clean `xcodebuild build`.
  2. `[SPRD-255][2/n]` — Added concrete `TaskCoordinator`/`NoteCoordinator` (`Spread/JournalManager/TaskCoordinator.swift`/`NoteCoordinator.swift`), covering all 18 of `JournalManager`'s current task/note CRUD + migration methods, depending only on `TaskRepository`/`NoteRepository` and `JournalRuleEngine` — no dependency on `JournalManager`, per the design decision in this task's Description (`spreads` passed in per call, no upsert callback; callers patch their own observed state from the mutated/returned entity). Purely additive — `JournalManager`'s own methods are untouched and don't call these yet (also flagged with `TODO: [SPRD-255]` comments). Added `TaskCoordinatorTests.swift`/`NoteCoordinatorTests.swift` (24 tests) covering creation (matched/unmatched spread), title/status/date-period/metadata updates, the `.migrated`-status-rejection error path, preferred-assignment clearing, deletion, and migration (cancelled-task rejection, missing-source-assignment rejection, Inbox-origin moves, batch migration skipping cancelled tasks, and historical-destination revival) — constructed directly with `TestTaskRepository`/`TestNoteRepository` + `JournalRuleEngine`, no `JournalManager` involved, since no legacy `Standard*` counterpart exists to parity-test against. Verified via `-only-testing:SpreadTests/TaskCoordinatorTests -only-testing:SpreadTests/NoteCoordinatorTests` (24/24 pass), a full `xcodebuild test` (1269 tests, only the pre-existing/unrelated `SpreadCardStyleTests.testTodayFillDistinct` failure), and a full clean `xcodebuild build`.
  3. `[SPRD-255][3/n]` — Wired `JournalManager`'s 18 task/note CRUD + migration methods to delegate to `taskCoordinator`/`noteCoordinator` (new private computed properties on `JournalManager`, constructed fresh on each access from `taskRepository`/`noteRepository` + the current `ruleEngine` — computed rather than stored, since `ruleEngine` itself is reassigned in `apply(snapshot:)` on day-boundary/calendar/time-zone/locale changes, and a stored coordinator would otherwise hold a stale copy of that value-type dependency). Each `JournalManager` method now does the `spreads` lookup it already had, delegates to the coordinator, then calls its own `upsertTask`/`upsertNote`/`removeTask`/`removeNote` to patch the incremental index — exactly the split this task's Description called for. Removed both coordinators' `TODO: [SPRD-255]` doc-comment markers now that they're wired in. Zero new tests needed — this increment is pure delegation with unchanged signatures and behavior. Verified via a full clean `xcodebuild build` (zero errors) and a full `xcodebuild test` (1269 tests, only the pre-existing/unrelated `SpreadCardStyleTests.testTodayFillDistinct` failure) — `JournalManager`'s own black-box suites (`JournalManagerTaskCRUDTests`, `MigrationTests`, `JournalManagerNoteTests`, etc.) all pass unmodified, confirming the extraction is behavior-preserving.
- Task complete — all ACs satisfied.

---

### [SPRD-256] Refactor: Additive concrete SpreadDeletionCoordinator - [x] Done

- **Staleness correction (2026-06-26, audited before implementation, same pattern as SPRD-255)**: this task's original Context/Description/ACs/Tests reference `StandardSpreadDeletionCoordinator`, `StandardSpreadDeletionPlanner`, and `SpreadDeletionCoordinatorTests`/`SpreadDeletionPlannerTests` — **all deleted in SPRD-251**, confirmed via grep (zero remaining references in `Spread/`/`SpreadTests/`; only two doc-comment *mentions* of the legacy planner survive, in `JournalManager.swift:679,700`, describing behavior the current code mirrors). The real current home of this logic is `JournalManager.deleteSpread` (`Spread/JournalManager/Journal Store/JournalManager.swift:574-677`) plus its private helpers `replacementSpread(for:deleting:parentSpread:)` (Task/Note overloads, lines 685-714) and `findParentSpread` (line 716). Rewritten below in terms of the current codebase.
- **Context**: `JournalManager.deleteSpread` reassigns every task/note's assignment on the deleted spread to a parent spread (day→month→year walk) or Inbox, touching `spreadRepository`, `taskRepository`, and `noteRepository` simultaneously — a genuinely distinct dependency shape and an infrequent lifecycle (spread deletion, not routine entry mutation) versus SPRD-255's `TaskCoordinator`/`NoteCoordinator`. Per CLAUDE.md's coordinator-extraction guidance ("distinct lifecycle, distinct dependencies"), this earns staying its own type.
- **Description**: Extract a concrete `SpreadDeletionCoordinator` (no protocol, no "Standard" prefix) covering `deleteSpread`'s current logic plus its two `replacementSpread` overloads and `findParentSpread`. Depends on `SpreadRepository`/`TaskRepository`/`NoteRepository` (protocols) and `Calendar` — **not** `JournalRuleEngine`: the original Description speculated a `JournalRuleEngine` dependency "if assignment reconciliation is needed," but the actual logic never calls a rule-engine reconciliation method, only `Assignment.matches(spread:calendar:)` and `SpreadService(calendar:).findBestSpread`, so pulling in `JournalRuleEngine` for its `calendar` property alone would be an unused dependency. Takes `spreads`/`tasks`/`notes` in per call (mirroring SPRD-255's coordinator shape) and returns the mutated tasks/notes for the caller to upsert into its own observed state — no back-reference to `JournalManager`. Entirely new file; the only edit to `JournalManager.swift` is replacing `deleteSpread`'s body with a delegating call and removing the two now-moved private helpers.
- **Spec**: `Documentation/Specs/JournalManager.md` — "Decision: Drop protocol-per-logic-seam; protocols are a repository-only boundary"
- **Acceptance Criteria**:
  - [x] New concrete `SpreadDeletionCoordinator` added, covering `JournalManager.deleteSpread`'s current responsibilities (parent-hierarchy reassignment, multiday fallback-to-best-spread, Inbox fallback, migration-history bookkeeping) plus the `replacementSpread`/`findParentSpread` helpers, depending only on `SpreadRepository`/`TaskRepository`/`NoteRepository` and `Calendar` — not on `JournalManager` itself.
  - [x] Does not declare or conform to a new protocol; no "Standard" naming.
  - [x] `JournalManager.deleteSpread`'s public signature is unchanged; its body becomes a thin delegation to the new coordinator plus the `upsertTask`/`upsertNote`/`removeSpread` patching the coordinator itself doesn't do.
  - [x] Project builds with no errors or warnings; full existing test suite (including all of `SpreadDeletionTests.swift`) passes unmodified.
- **Tests**:
  - [x] Exhaustive unit tests for `SpreadDeletionCoordinator`, constructing it directly with `Test*Repository` doubles, mirroring the scenarios in `SpreadDeletionTests.swift` that test deletion/reassignment logic itself (parent reassignment for tasks/notes, Inbox fallback, history preservation, completed/migrated/multiple-task reassignment, grandparent fallback, multiday fallback-to-best-spread vs. fallback-to-Inbox, events unaffected) — excluding the 3 scenarios that are `JournalManager`-index-specific (`testDeleteSpreadIncrementsDataVersion`, `testDeleteSpreadRebuildDataModel`, `testDeleteSpreadFailureKeepsSpreadInMemory`), which stay as `JournalManager`-only tests since they test index/observed-state behavior the coordinator doesn't own. No legacy `Standard*` counterpart exists to parity-test against.
  - [x] Confirm `JournalManager`'s own existing test suites (`SpreadDeletionTests`, etc.) still pass unmodified after the delegation.
- **Dependencies**: SPRD-245 (repository layer, done), SPRD-248 (`JournalRuleEngine`, done — not actually used by this task's coordinator; see Description).
- **Progress (commits landed on feature/SESH-24)**:
...
- **Progress (commits landed on feature/SESH-25)**:
  ...
  2. `[SPRD-258][2/n]` — Follow-up enhancement beyond the original AC scope, per user request: added `.type` to both `EntryGroupingOption` (keys by `Entry.entryType.displayName`, reusing the existing alphabetical `sortedKeysUntitledLast` bucket ordering) and `EntrySortOption` (orders by `EntryType`'s declaration order — task, event, note — via a `typeRank` helper mirroring the existing `priorityRank` pattern). Added 2 tests (`testTypeGroupsByEntryType`, `testTypeOrdersByEntryTypeDeclarationOrder`) covering a mixed Task/Note/Event entry list. Verified via `-only-testing:SpreadTests/EntryGroupingOptionTests -only-testing:SpreadTests/EntrySortOptionTests` (10/10 pass) and a full `xcodebuild test` (1296/1296 pass) plus a full clean `xcodebuild build`.
  3. `[SPRD-258][3/n]` — Follow-up per user request: every non-`.manual`, non-`.title` `EntrySortOption` (`.priority`, `.dueDate`, `.type`) now breaks ties on its primary key by alphabetical title, via a new `withTitleTiebreaker(_:)` helper that wraps a primary comparator and falls back to the existing `titleAscending` comparator when neither entry sorts before the other under the primary key. Without this, two same-priority/same-due-date/same-type entries fell back to whatever order `sorted(by:)` happened to preserve them in, which is unspecified. Added 3 tests (`testPriorityTiesBreakByTitleAlphabetically`, `testDueDateTiesBreakByTitleAlphabetically`, `testTypeTiesBreakByTitleAlphabetically`). Verified via `-only-testing:SpreadTests/EntrySortOptionTests` (8/8 pass) and a full `xcodebuild test` (1298/1298 pass) plus a full clean `xcodebuild build`.
- Task complete — all ACs satisfied.

---

### [SPRD-259] Feature: Adopt grouping/sorting picker in Day spread - [x] Done
...
- **Tests**:
  ...
  - [x] Manual verification: spot-checked in the simulator on the Baseline data set (Debug menu) — group-by/order-by re-partition the Day spread list as expected. The user judged this sufficient without exhaustively walking every option/tiebreak/persistence combination from the full checklist (see `[SPRD-259][4/n]` below) — full automated coverage of grouping/sorting logic exists in `EntryGroupingOptionTests.swift`/`EntrySortOptionTests`/`EntryListGroupingTests.swift`.
...
- **Progress (commits landed on feature/SESH-25)**:
  ...
  2. `[SPRD-259][2/n]` — Enriched `MockDataSet.baseline`'s today-dated tasks/notes (`Spread/Debug/MockDataSet.swift`, `#if DEBUG`-only, localhost scheme) to exercise group-by/order-by manually in the simulator: added a 4th list ("Health") and 5th tag ("Urgent"); gave each existing today-task a distinct time-of-day (was all `date: today` with identical instants, so `.dueDate` sort had no visible effect — `EntrySortOption.dueDate` sorts by `sortDate`, which ties when every entry shares the exact same `Date` value); added 4 new tasks and 1 new note covering previously-missing combinations: `.low` priority, `.cancelled` status, a task with no list/no tags (the "Untitled" fallback bucket for both `.list` and `.tag` grouping), and a task/note with a list but no tags (the "Untitled" tag bucket specifically). No behavior/logic changes — data only. Verified via a full `xcodebuild test` (1296/1296 pass, no regressions — `MockDataSetTests`'s baseline assertions are non-empty/contains checks, not exact counts) and a full clean `xcodebuild build`.
  3. `[SPRD-259][3/n]` — UI polish on `EntryListOptionsPicker`/Day's toolbar row, per follow-up feedback: added an `EntryListOptionsPicker.Config` (nested struct, `systemImageName`/`accessibilityLabel`, `static let default`) so callers can customize the control's icon/label without touching its menu structure — mirrors the existing `DaySpreadContentView.Config` pattern. Each option group is its own titled submenu (`Menu("Group By") { ... }` / `Menu("Order By") { ... }`) nested inside the top-level `Menu`, rather than relying on a bare `Picker`'s label being inferred as a section header. The component's API was changed from `@Binding var grouping`/`@Binding var sorting` (backed by a `Picker` per group) to plain `let grouping`/`let sorting` values plus injected `onGroupingSelected`/`onSortingSelected` closures (backed by one `Button` per option, with the checkmark drawn explicitly for the currently-selected option) — per follow-up feedback that a reusable component shouldn't require its caller's selection state to be a literal `Binding`; callers (e.g. `@AppStorage`-backed call sites) now just pass the current value and a closure that writes the new one however they choose. `DaySpreadContentView`'s call site updated accordingly. Applied `.buttonStyle(.plain)` + `.contentShape(Rectangle())` to the picker's outer `Menu` to match Day's favorite/edit buttons' plain-icon styling. In `DaySpreadContentView`, removed the edit button's ad hoc `.padding(SpreadTheme.Spacing.large)` and moved an equivalent trailing inset to the outer `HStack` instead, so the picker/favorite/edit trio are now evenly spaced by the `HStack`'s own `spacing:` alone rather than one button carrying extra asymmetric padding. Verified via a full `xcodebuild test` (1298/1298 pass) and a full clean `xcodebuild build`.
  4. `[SPRD-259][4/n]` — Manual simulator verification (2026-06-27): built and installed a fresh debug build on the iOS Simulator (iPhone 17 Pro), loaded `MockDataSet.baseline` via the Debug menu, and spot-checked the Day spread's `EntryListOptionsPicker`. User confirmed it "looks good enough for now" without working through every item on the full verification checklist (all Group By/Order By combinations, the priority tiebreak, quick-add anchoring across grouping modes, and relaunch persistence) — accepted as sufficient to proceed; not an exhaustive pass.
- Task complete — all ACs satisfied; manual verification accepted by user at a spot-check level rather than exhaustively.

---

### [SPRD-260] Feature: Adopt grouping/sorting picker in Month, Year, MonthCard, and Multiday - [x] Done
...
- **Acceptance Criteria**:
  ...
  - [x] Multiday applies the same grouping/sorting enums to its flat entry list before rendering, without requiring `EntryListView` adoption. **Scope decision**: `groupingOption` only subdivides the leading "This Range" (multiday-assigned) section into one card per group — per-day cards are a fixed, structural grouping by date already, and the compact day-card layout has no room for a second nested grouping dimension inside it. `sortingOption` applies everywhere: within "This Range" and within every day card.
  - [x] Each spread type's persisted choice is independent (changing Day's grouping does not affect Month/Year/Multiday).
  - [x] Project builds with no errors or warnings.
- **Tests**:
  - [x] Manual verification per spread: toggling group-by/order-by re-partitions/re-orders correctly; persistence is independent per spread type. Accepted by the user without a further exhaustive simulator pass (see `[SPRD-260][5/n]` below) — automated coverage exists in `EntryListGroupingTests.swift`/`EntryGroupingOptionTests.swift`/`EntrySortOptionTests`.
...
- **Progress (commits landed on feature/SESH-25)**:
  ...
  3. `[SPRD-260][3/n]` — Pre-existing bug found while testing this task (not caused by it, but in the same file): `YearSpreadContentView.body` had no scroll container at all — a bare `LazyVStack` with padding, unlike `MonthSpreadContentView` (`ScrollView { ScrollViewReader { LazyVStack {...} } }`) and `MultidaySpreadContentView` (`.conditionalScrollView()`). Fixed by wrapping the `LazyVStack` in a plain `ScrollView`, matching Month's pattern exactly (no `ScrollViewReader` needed — Year has no programmatic scroll-to-section feature). Verified via a full `xcodebuild test` (1298/1298 pass) and a full clean `xcodebuild build`. **Visual confirmation of the scroll fix itself was attempted but not reliably completed** — GUI automation in this environment produced inconsistent screenshots between the compositor and actual device state; the fix is high-confidence by direct structural parity with Month's already-working pattern, but ask the user to confirm scrolling on a real device/simulator before considering this fully closed.
  4. `[SPRD-260][4/n]` — Added `@AppStorage("entryGrouping.multiday")` (default `.none`, not `.list` like the other spreads — see scope decision above; defaulting to `.none` keeps the existing single "This Range" card as the out-of-the-box look) and `@AppStorage("entrySorting.multiday")` (default `.dueDate`) to `MultidaySpreadContentView`, rendered in a new header row above the grid (inside the same scrollable region, with `.padding(.horizontal, SpreadTheme.Spacing.large)` on the picker matching the manual correction already applied to Month/Year). `ViewModel.sections` became `sections(groupedBy:orderedBy:)`, and `makeSections` now takes `groupingOption`/`sortingOption`: the "This Range" bucket is built via `EntryList.Section.grouped(from:by:orderedBy:)` and expanded into one `EntryList.Section` per group (titled `"This Range — \(group)"`, or plain `"This Range"` for `.none`); each day bucket keeps its existing fixed per-day grouping but is now ordered via `sortingOption.areInOrder` (falling back to the prior hardcoded `sortDate` ordering for `.manual`, preserving default behavior exactly). Updated 7 pre-existing test call sites (5 in `EntryListGroupingTests.swift`, 2 in `MultidaySpreadUITests.swift`) to the new signature with `.none`/`.dueDate` (reproducing prior behavior exactly), and added 2 new tests (`testMultidayRangeSectionSubdividesByGrouping`, `testMultidayDayCardEntriesFollowSortingOption`). Verified via `-only-testing:SpreadTests/EntryListGroupingTests` (13/13 pass) and a full `xcodebuild test` (1301/1301 pass, no regressions) plus a full clean `xcodebuild build`.
  5. `[SPRD-260][5/n]` — Manual verification (2026-06-27): user accepted the implementation without an exhaustive simulator pass across Month/Year/MonthCard/Multiday, relying on automated test coverage instead. A follow-up UI tweak attempt this session (relocating the picker into the "This Range" card and matching its height to the first day-card row) was started but reverted at the user's request ("forget this for now") before landing — `MultidaySpreadContentView`/`MultidaySpreadContentView+ViewModel.swift` are unchanged from `[4/n]`'s committed state. Revisit those two UI tweaks as a separate follow-up task if still wanted.
- Task complete — all ACs satisfied; manual verification accepted by user at a non-exhaustive level, consistent with SPRD-259's closure.
...
- **Progress (commits landed on feature/SESH-25)**:
  ...
  2. `[SPRD-267][2/n]` — Applied `largeTitle` to the Day spread's title specifically, per follow-up request: `SpreadContentPagerView.spreadDetailTitle` (the shared title shown above the pager for whichever spread is currently selected — Year/Month/Day/Multiday) now picks its font via a new `titleFont(for:)` helper, using `SpreadTheme.Typography.largeTitle(size: 17, weight: .bold)` when `currentSelection.period == .day` and the existing `SpreadTheme.Typography.heading(size: 17, weight: .semibold)` (Avenir Next) for every other period — unchanged for Year/Month/Multiday. `.bold` chosen over `.semibold` since only Bold/Regular are bundled (no continuous weight axis) and Bold reads closer to the prior semibold emphasis than Regular would. Verified via a full `xcodebuild test` (1318/1318 pass, no regressions) and a full clean `xcodebuild build`. **Visual confirmation not completed** — couldn't reliably navigate the simulator to a Day spread via available automation this session (default launch lands on the Year spread); recommend the user check the Day spread's title visually before considering this fully closed.
- Task complete — all ACs satisfied for the original scope; the Day-title application above is a follow-up addition pending the user's own visual check.

---

### [SPRD-268] Feature: Standardize SpreadTheme typography on Apple's 11-style scale + Mulish headings - [ ] In Progress

- **AC correction (2026-06-28, found during implementation)**: the spec/AC text assumed Mulish would need "separate static weight files" mirroring Fuzzy Bubbles. Checked the actual `google/fonts` repo first: Mulish ships as a **single variable font** (`Mulish[wght].ttf`, continuous `wght` axis 200–1000) with named instances at standard weight stops, not separate static files. Bundled that one file (`Mulish-Variable.ttf`) instead of four. Still maps each requested `Font.Weight` to an explicit named-instance PostScript name rather than `.weight()`-chaining on `Font.custom` (consistent with the Fuzzy Bubbles precedent — relying on `.weight()` for a third-party variable font is less proven than an explicit name lookup). The exact instance names were **not** guessed — confirmed empirically via a temporary diagnostic test calling `UIFont.fontNames(forFamilyName: "Mulish")`, which is how the `MulishRoman-{Weight}` naming (not `Mulish-{Weight}`) was discovered; the diagnostic test was removed before committing.
- **Context**: SPRD-267 added a bundled font (Fuzzy Bubbles) for `largeTitle` only, and `SpreadTheme.Typography` has accumulated 10 of Apple's 11 standard Dynamic Type styles organically rather than by design — `headline` is missing despite already being used directly (and inconsistently) in 4+ files, and ~24 files across the app bypass `SpreadTheme.Typography` entirely with raw `.font(.body)`/`.title2.weight(.semibold)`/etc. The user wants to formalize this into an explicit, scalable typography system before the gap widens further, using an actual industry-standard model as the foundation rather than an ad hoc one.
- **Description**: Complete `SpreadTheme.Typography`'s scale to all 11 of Apple's standard Dynamic Type styles (add the missing `headline`). Replace Avenir Next with a newly-bundled **Mulish** (Google Fonts, OFL-licensed) for `title`/`title2`/`title3` and the `heading(size:weight:)` function — chosen because it's portable/embeddable rather than tied to Apple's system font catalog. `largeTitle` stays on Fuzzy Bubbles (SPRD-267), unchanged — a deliberate two-font exception, not something this task reconciles. `body`/`callout`/`subheadline`/`footnote`/`caption`/`caption2` stay on system Dynamic Type styles, unchanged. Migrate all ~24 existing direct/raw font call sites across the app onto the corresponding `SpreadTheme.Typography.*` member. Document the "always use `SpreadTheme.Typography`" convention; no SwiftLint enforcement in this task (explicitly deferred).
- **Spec**: `Documentation/Specs/DesignSystem.md` — "Typography System Standardization (SPRD-268)"
- **Acceptance Criteria**:
  - [x] `SpreadTheme.Typography` has all 11 of Apple's Dynamic Type styles, including a newly-added `headline`.
  - [x] `title`/`title2`/`title3` and `heading(size:weight:)` use the bundled Mulish font instead of Avenir Next; Mulish font file + OFL license added under `Spread/Resources/Fonts/`, registered in `Info.plist`'s `UIAppFonts`. Explicit PostScript-name mapping per `Font.Weight` (not `.weight()` chaining) — see AC correction above re: one variable file, not separate static files.
  - [x] `largeTitle` remains on Fuzzy Bubbles (SPRD-267) — unchanged.
  - [x] `body`/`callout`/`subheadline`/`footnote`/`caption`/`caption2` remain on system Dynamic Type styles — unchanged.
  - [x] All ~24 existing direct `.font(.body)`/`.title`/`.title2`/`.title3`/`.headline`/`.caption`/etc. call sites across the app are migrated to the corresponding `SpreadTheme.Typography` member, with no visual/size/weight regression.
  - [x] The "always use `SpreadTheme.Typography`, never a raw system text style" convention is documented (`CLAUDE.md` and/or `DesignSystem.md`).
  - [x] Project builds with no errors or warnings.
- **Tests**:
  - [x] Unit tests confirming each bundled Mulish weight's PostScript name resolves via `UIFont(name:size:)` — mirrors `SpreadThemeTests.swift`'s existing Fuzzy Bubbles registration tests.
  - [x] Unit test(s) confirming `heading(size:weight:)` resolves to the correct Mulish PostScript name per requested weight, including the fallback behavior for an unmapped weight (`.black`).
  - [ ] Manual/visual verification (existing previews where available) that migrated call sites render with unchanged size/weight/hierarchy.
- **Progress (commits landed on feature/SESH-25)**:
  1. `[SPRD-268][1/n]` — Bundled `Mulish-Variable.ttf` (+ `Mulish-OFL.txt`) at `Spread/Resources/Fonts/`, registered in `Info.plist`'s `UIAppFonts`. Added `SpreadTheme.FontFamily.headingRegular`/`.headingMedium`/`.headingSemiBold`/`.headingBold` (the empirically-confirmed `MulishRoman-{Weight}` named-instance PostScript names) and rewrote `Typography.heading(size:weight:)` to map `Font.Weight` to one of them directly (falling back to Regular for unmapped weights), replacing the prior `Font.custom("Avenir Next", size:).weight(weight)`. Added `Typography.headline` (system Dynamic Type style), completing the 11-style scale. `title`/`title2`/`title3`'s own sizes/weights are unchanged — only the underlying font family/lookup mechanism changed underneath `heading(size:weight:)`. Added 7 new tests to `SpreadThemeTests.swift` (Mulish registration via a parameterized test, weight-to-instance mapping, the unmapped-weight fallback, and a regression check that `title`/`title2`/`title3` still resolve to their expected size/weight). Verified via `-only-testing:SpreadTests/SpreadThemeTests` (9 tests, 12 including parameterized cases, all pass) and a full `xcodebuild test` (1322/1322 pass, no regressions) plus a full clean `xcodebuild build`; confirmed `Mulish-Variable.ttf` is present in the built `.app` bundle.
  2. `[SPRD-268][2/n]` — Migrated every direct/raw font usage onto `SpreadTheme.Typography` across 36 files (more than the originally-estimated ~24, found via a repo-wide `grep`): `.font(.body)` → `SpreadTheme.Typography.body`, `.font(.headline)` → `.headline`, `.font(.caption.weight(.semibold))` → `SpreadTheme.Typography.caption.weight(.semibold)`, etc. Two non-mechanical cases: `OnboardingSheet`'s `.title2.weight(.semibold)` collapsed onto `Typography.title2` directly (already semibold/20pt — exact match); `CollectionEditorView`'s `.title2.bold()` mapped to `SpreadTheme.Typography.heading(size: 20, weight: .bold)` since `Typography` has no pre-built bold `title2` token. Left `.font(.system(size:))` fixed-pixel usages (calendar grid digits in `MonthCardView`, `FullMonthCalendarContentGenerator`, `MultidaySpreadContentView`, `OnboardingSheet`'s icon) untouched — out of scope, not part of the Dynamic Type hierarchy. Documented the "always use `SpreadTheme.Typography`" convention in `CLAUDE.md`'s Code Style Guide (Patterns section) and added a "Migration complete" note to `DesignSystem.md`. Verified via a full `xcodebuild build` (success) and full `xcodebuild test` (1322/1322 pass, no regressions).
- Remaining for this task: manual visual verification that migrated call sites render with unchanged size/weight/hierarchy.

### [SPRD-269] Feature: Replace SF Symbols with Phosphor icons via a new SpreadTheme.Icon namespace - [ ] In Progress

- **Context**: Following the same "single source of truth" pattern SPRD-267/268 established for fonts (`SpreadTheme.Typography`), the user wants the app's SF Symbol icons replaced with the Phosphor icon set for a more distinct, illustrated visual identity. An audit found 99 SF Symbol call sites across ~41 files, several of them indirect — `String`-typed `systemImage`/`icon` computed properties on `SyncStatus`, `ConvenienceNavigationButtonState`, `RootNavigationView+Content`, `Action`'s menu-label icons, `EntryListOptionsPicker.Config`, and `DebugRepositoryListView`'s per-entity-type icon helpers — rather than literal `Image(systemName: "...")` strings at the render site.
- **Description**: Add [`phosphor-icons/swift`](https://github.com/phosphor-icons/swift) (official Phosphor SwiftUI port, MIT-licensed, product name `PhosphorSwift`) as a remote SPM dependency, wired directly into `Spread.xcodeproj/project.pbxproj` (mirroring the existing `supabase-swift` `XCRemoteSwiftPackageReference`/`XCSwiftPackageProductDependency` entries — this repo has no `Package.swift`). Introduce `SpreadTheme.Icon`, a semantic enum mirroring `SpreadTheme.Typography`'s role, where each case resolves to a Phosphor `Image` at a default weight (SF Symbol outlines → Phosphor `.regular`; existing `*.fill` SF Symbols → Phosphor `.fill`). Migrate every SF Symbol usage in the app — including `Spread/Debug/*` — onto `SpreadTheme.Icon`, including the `Tab(_:systemImage:value:)`/`Label(_:systemImage:)` call sites (`RootNavigationView`'s `TabView`, `SpreadHeaderView`, `Action`'s menu labels, `DebugRepositoryListView`), which switch to the `Image`-based label-closure forms since Phosphor returns `Image`, not a symbol-name `String`.
- **AC correction (2026-06-28, found during implementation)**: Phosphor icons are plain resizable `Image`s, not font glyphs — they don't inherit ambient `.font()` sizing or `.foregroundStyle()`/`.tint()` color the way SF Symbols do. Added `SpreadTheme.Icon.sized(_:)` (explicit square-frame sizing, defaulting to `SpreadTheme.IconSize.medium`) and a local `.iconTint(_:)` view modifier (a re-implementation of PhosphorSwift's own `.color(_:)` blend modifier, kept local so call sites depend only on `SpreadTheme.Icon` and never need to `import PhosphorSwift` directly) — every migrated call site now sizes/colors explicitly rather than relying on now-absent implicit behavior. Also: `.symbolEffect(.rotate:)` (used by `SyncIconButton` for the spinning sync icon) is SF-Symbol-only and has no Phosphor equivalent — replaced with a manual `.rotationEffect` driven by a `repeatForever` animation toggled on the syncing state.
- **Spec**: `Documentation/Specs/DesignSystem.md` — "Icon System: SF Symbols → Phosphor (SPRD-269)"
- **Acceptance Criteria**:
  - [x] `phosphor-icons/swift` is added as a pinned (`upToNextMajorVersion` from `2.0.0`) remote SPM dependency; project builds with no errors or warnings.
  - [x] `SpreadTheme.Icon` exists with a case for every distinct icon currently in use (63 cases — more than the originally-estimated set, several discovered mid-migration: `person.crop.circle`/`.fill`, `doc.text`, `arrow.left.arrow.right`, `testtube.2`), each resolving to the correct Phosphor `Image` + weight (`.regular` for outlines, `.fill` for `*.fill` SF Symbols).
  - [x] Every `Image(systemName:)`/`systemImage:` usage across the app (including `Spread/Debug/*`) is migrated to `SpreadTheme.Icon`, with no SF Symbol strings remaining outside the mapping itself. Verified via a repo-wide `grep` for `Image(systemName:`/`systemImage:`/`symbolEffect` returning zero production matches.
  - [x] `RootNavigationView`'s `TabView`, `SpreadHeaderView`, `Action`'s menu labels, and `DebugRepositoryListView` render their icons via the `Image`-based label-closure forms instead of `systemImage:`.
  - [ ] No visual regression in icon sizing/color/active-vs-inactive state at any migrated call site — pending manual visual verification (see Tests).
- **Tests**:
  - [x] Unit tests confirming each `SpreadTheme.Icon` case resolves to the expected Phosphor `Ph` case + weight (`SpreadThemeIconTests.swift`, mirrors `SpreadThemeTests.swift`'s font-mapping test style) — also deleted `SyncStatus.systemImage` (dead code, zero production callers, superseded by `.iconName`) and its now-pointless test.
  - [ ] Manual/visual verification (existing previews where available) that migrated icons render with unchanged size/color/state semantics.
- **Progress (commits landed on feature/SESH-25)**:
  1. `[SPRD-269][1/n]` — Added `phosphor-icons/swift` SPM dependency.
...
### [SPRD-270] Visual: Restyle the cancelled-status slash overlay to extend beyond the circle like the migrated-status arrow - [x] Done
...
- **AC correction (found during implementation)**: First pass just grew `.slash`'s own `frameSize` multiplier in isolation (`s * 2.2`, comfortably past the `s / 0.6 ≈ 1.667 * s` threshold needed for `SlashShape`'s old 20%–80%-of-rect path to clear the circle). The user then explicitly asked for component reuse rather than a standalone numeric tweak: make `SlashShape` (in `johnnyo-foundation`) structurally match `XMarkShape` — centered on the rect, scaled by an `armLength` parameter — and configure `.slash` in `EntryStatusIcon` with `.xmark`'s exact `decoratorSize`/`armLength` formula (`decoratorSize = s * (1 + 2 * 0.35)`, `armLength = decoratorSize * 0.6`). Landed there instead; the `s * 2.2` approach was replaced, not layered on top.
- **Spec**: `Documentation/Specs/EntryComponents.md` — "Decision: `.slash` mirrors `.xmark`'s shape/configuration structure, not a standalone frameSize tweak"
- **Acceptance Criteria**:
  - [x] `SlashShape` takes an `armLength: CGFloat` and draws one diagonal arm (top-right → bottom-left) centered on `rect.midX`/`rect.midY` — the same construction pattern as `XMarkShape`.
  - [x] `EntryStatusIcon.overlayView`'s `.slash` case computes `decoratorSize`/`armLength`/`frameSize` with the identical formula `.xmark` uses; `strokeStyle`/`animationDuration` stay `.slash`-specific (unchanged), since those are legitimately different visual weights/timings, not part of what needed to match.
  - [x] `overlayAlignment` is unchanged (`.slash` still resolves to `.center`) — no new `OverlayShape` case, `BaseShape` case, or view type was added.
  - [x] No other `SlashShape(...)` call site needed updating for the new required `armLength` argument — confirmed via `grep` that `EntryStatusIcon.swift`'s `.slash` case is the only call site.
  - [x] Existing `#Preview` blocks in `EntryStatusIcon.swift` (`"Task Statuses"`, `"Sizes"`) continue to compile and show the resized cancel icon. Verified via full `xcodebuild build`.
- **Tests**:
...
### [SPRD-271] Perf: Cache MonthCalendarModel and row-overlay layouts per month - [x] Done
...
- **Acceptance Criteria**:
  - [x] Re-rendering `CalendarView` with the same visible months and unchanged inputs does not re-run `MonthCalendarModelBuilder.makeModel` or `MonthCalendarRowOverlayLayoutBuilder.makeWeekLayouts` for those months a second time (verified via a call-counting test double or instrumentation, not just visual inspection).
  - [x] Changing `displayedMonth`, `calendar`, `configuration`, or the overlay set for a month still produces a freshly correct model/layout for that month (no stale-cache bugs).
  - [x] `MonthCalendarView`'s public init signature and existing call sites (`CalendarView.swift`, `SpreadsNavigatorView.swift`) are unchanged.
  - [x] Project builds with no errors or warnings; full existing `johnnyo-foundation` and `Spread` test suites pass unmodified.
- **Tests**:
  - [x] New unit test(s) in the `johnnyo-foundation` package asserting `MonthCalendarModelBuilder.makeModel` is not re-invoked (e.g. via a counting wrapper or cache-hit assertion) when called twice with identical inputs, and is correctly invoked again when an input changes.
  - [x] Equivalent test(s) for `MonthCalendarRowOverlayLayoutBuilder.makeWeekLayouts`.
  - [x] Existing `MonthCalendarView`/`CalendarView` rendering tests (if any) pass unmodified, confirming the cache is behavior-preserving.
- **Architectural decision (confirmed with user 2026-06-28)**: the row-overlay layout cache (`MonthCalendarRowOverlayLayoutBuilder.makeWeekLayouts`) is generic over `OverlayPayload: Sendable` with no `Hashable` requirement, but the function's output embeds the actual overlay value (including payload) passed in. Caching keyed only on `(OverlayID, startDate, endDate)` — the only fields hashable today — would silently return a stale payload if the same id/date-range recurred with a different payload value. User chose to tighten the generic constraint to `OverlayPayload: Hashable & Sendable` (across `MonthCalendarLogicalRowOverlay`, the packed/overflow render contexts, `MonthCalendarRowOverlayLayoutBuilder`, and the `MonthCalendarRowOverlayGenerator` protocol's associated type) so the cache can key on the full overlay value safely. Today's only concrete usage (`MonthCalendarLogicalRowOverlay<UUID, Bool>` in `SpreadsNavigatorView+CalendarGenerator.swift`) and the package's own test usages (`String` payloads) already satisfy `Hashable`, so no real call site breaks. This constraint-tightening is scoped to a later increment in this same task, not yet landed.
- **Progress (commits landed on feature/SESH-26)**:
  1. `[SPRD-271][1/n]` — Added a lock-protected memoization cache to `MonthCalendarModelBuilder.makeModel` (`johnnyo-foundation/Sources/JohnnyOFoundationCore/Calendar/MonthCalendarModelBuilder.swift`), keyed on `(normalizedMonth, calendar, configuration, today)`. Added `Hashable` conformance to `MonthCalendarConfiguration` (a single `Bool`) to support the key. Cache storage lives in a private `final class Cache: @unchecked Sendable` guarded by an `NSLock`, since `makeModel` is called synchronously from `View.init` rather than from async/actor context — documented inline why the `@unchecked Sendable` is safe. Exposed `internal static var buildCountForTesting`/`resetCacheForTesting()` (not `public`, reachable only via `@testable import`) so tests can assert cache-hit behavior without adding test-only state to the public API surface. Did not touch `MonthCalendarRowOverlayLayoutBuilder` — that's scoped to a later increment per the architectural decision above, since it needs the `Hashable` constraint tightening first. `MonthCalendarView`'s public init signature and all call sites unchanged. Added 3 new tests (`testMakeModelCachesIdenticalInputs`, `testMakeModelRebuildsWhenInputsChange`, `testMakeModelNormalizesDisplayedMonthBeforeCaching`) to `MonthCalendarModelBuilderTests.swift`, marking the suite `@Suite(.serialized)` since Swift Testing runs `@Test` functions in parallel by default and these tests assert on the shared static cache/counter. Verified via `swift test` in `johnnyo-foundation` (47/47 pass) and a full `xcodebuild build` for `Spread Localhost` (the actual scheme name in this checkout — `CLAUDE.md`'s documented `-scheme Spread` is stale).
  2. `[SPRD-271][2/n]` — Tightened the `OverlayPayload` generic constraint from `Sendable` to `Hashable & Sendable` across `MonthCalendarLogicalRowOverlay`, `MonthCalendarPackedRowOverlayRenderContext`, `MonthCalendarOverflowedRowOverlaySegment`, `MonthCalendarRowOverlayOverflowRenderContext`, `MonthCalendarPackedRowOverlayWeekLayout`, every generic function in `MonthCalendarRowOverlayLayoutBuilder` (including the private `NormalizedOverlay`/`RowSegment` helper types), and the `MonthCalendarRowOverlayGenerator` protocol's associated type (plus its built-in `MonthCalendarEmptyRowOverlayPayload`). No call site needed changes — `Spread`'s only concrete usage (`MonthCalendarLogicalRowOverlay<UUID, Bool>`) and the package's own test usages (`String` payloads) already satisfy `Hashable`. This increment is purely a constraint change with zero behavior change — no new tests needed, existing suite is the regression check. Verified via `swift test` (47/47 pass) and a full `xcodebuild build` for `Spread Localhost`.
  3. `[SPRD-271][3/n]` — Added the memoization cache to `MonthCalendarRowOverlayLayoutBuilder.makeWeekLayouts`, keyed on `(overlays, model, calendar, maximumVisibleLaneCount)`. Added `Hashable` conformance to `MonthCalendarModel`/`MonthCalendarWeek`/`MonthCalendarSlot` (`MonthCalendarModel.swift`) and to `MonthCalendarLogicalRowOverlay` itself, so the key can include the full model and overlay values rather than a hand-rolled proxy. Swift doesn't support static stored properties directly inside a generic type, so cache storage is type-erased behind one non-generic `Cache` singleton keyed by an `ObjectIdentifier`-pair of `(OverlayID, OverlayPayload)` plus the typed `CacheKey`, with each call site casting back to its own concrete dictionary type — documented inline why that cast is always safe (the type pair used to build the registry key is the same one used to build the cast target). Exposed `static func buildCountForTesting(overlayID:overlayPayload:)`/`resetCacheForTesting(overlayID:overlayPayload:)` (internal, explicit type parameters since they can't be inferred from no arguments) mirroring SPRD-271[1/n]'s test-hook pattern. `MonthCalendarView`'s public init signature and all call sites unchanged. Added 2 new tests (`testMakeWeekLayoutsCachesIdenticalInputs`, `testMakeWeekLayoutsRebuildsWhenPayloadOrLaneCountChanges` — the latter specifically proving the same id/date-range with a different payload value is never served stale, the risk flagged in the architectural decision above). Verified via `swift test` in `johnnyo-foundation` (49/49 pass) and a full `xcodebuild build` for `Spread Localhost`.
- Task complete — all ACs satisfied.
- **Dependencies**: None — purely additive within `johnnyo-foundation`, no dependency on SPRD-272 or SPRD-273.

### [SPRD-272] Perf: Avoid forced multi-month layout from the navigator's initial scrollTo - [x] Done
...
- **Spec**: `Documentation/Specs/CalendarFoundation.md`
- **Acceptance Criteria**:
  - [x] Opening the Spreads navigator scrolled to a month other than the first no longer forces synchronous layout of every intervening month — addressed structurally by switching from `ScrollViewReader.scrollTo` to SwiftUI's `.scrollPosition(id:anchor:)`, which is Apple's documented mechanism specifically for setting an initial/programmatic scroll position against a `LazyVStack` item's `id` without resolving geometry through every intervening item (unlike `scrollTo`, which requires realizing unrealized items to compute their offset). See the testing-gap note below — this AC is **not independently proven by an automated test** in this increment, since the package has no hosted-live-scroll test harness to observe `LazyVStack` realization under actual scrolling.
  - [x] The calendar still visibly opens scrolled to `today` (or the configured `initialScrollTarget`), matching current user-visible behavior exactly.
  - [x] `CalendarView`'s public init signature is unchanged.
  - [x] Project builds with no errors or warnings; full existing `johnnyo-foundation` and `Spread` test suites pass unmodified.
- **Tests**:
  - [ ] New unit/UI test asserting the number of `MonthCalendarView` instantiations triggered by initial appearance with a target several months in is bounded (e.g. only the target month ± a small visible window), not proportional to months-since-range-start. **Not implemented** — `MonthCalendarViewTests.swift`'s existing harness uses `ImageRenderer` for offscreen single-view rendering, which doesn't drive a real `ScrollView`'s viewport/scroll-offset machinery, so it can't distinguish "only the visible window realized" from "everything realized" for a `LazyVStack` the way a live-hosted window could. Verifying this claim is left to manual verification (the original user-reported symptom) rather than a fabricated test that wouldn't actually exercise the lazy-realization behavior in question.
  - [x] Existing scroll-to-today behavior test (or a new one if none exists) confirming the visible scroll position matches `today` after appearance — covered indirectly via 3 new unit tests on the extracted pure `resolvedCalendarScrollTarget` free function (`resolvedScrollTargetNormalizesToFirstOfMonth`, `resolvedScrollTargetIsNilWhenInitialTargetIsNil`, `resolvedScrollTargetIsNilWhenTargetOutsideRange`), proving the value fed into `.scrollPosition(id:)` is correct. The actual on-screen scroll behavior itself is not asserted by an automated test (same hosted-window limitation as above) — manual verification recommended.
- **Dependencies**: None directly, but should land after or alongside SPRD-271 since fixing this without the model/layout cache still leaves a (smaller) burst of rebuild work for the realized window. Landed after SPRD-271 in this session.
- **Progress (commits landed on feature/SESH-26)**:
  1. `[SPRD-272][1/n]` — Replaced `CalendarView`'s `ScrollViewReader` + `.onAppear { DispatchQueue.main.async { proxy.scrollTo(...) } }` pattern (`johnnyo-foundation/Sources/JohnnyOFoundationUI/Calendar/CalendarView.swift`) with a `@State private var scrollPosition: Date?` bound via `.scrollPosition(id: $scrollPosition, anchor: .center)`, seeded at construction time (`_scrollPosition = State(initialValue: ...)`) rather than after first appear. Extracted the target-normalization logic into a free function `resolvedCalendarScrollTarget(initialScrollTarget:startDate:endDate:calendar:months:)` (mirroring the existing `monthDateRange` free-function convention) so it (a) can run before `self` is fully initialized in `init`, and (b) is testable without specializing `CalendarView`'s generic parameters. Also tightened the range guard to check `initialScrollTarget` itself against `(startDate...endDate)` rather than the previous code's check against `today` (which could diverge from the target in theory, though all current callers pass `initialScrollTarget: today`). `CalendarView`'s public init signature unchanged. Added 3 new tests to `CalendarViewTests.swift` for `resolvedCalendarScrollTarget`. See the testing-gap notes above for what's deliberately *not* covered by automated tests. Verified via `swift test` in `johnnyo-foundation` (52/52 pass) and a full `xcodebuild build` for `Spread Localhost`.
- Task complete — all ACs satisfied, with the one documented testing-tooling gap noted above.

### [SPRD-273] Perf: Memoize SpreadsNavigatorView.selectedYearSpreads - [x] Done
...
- **Spec**: `Documentation/Specs/CalendarFoundation.md`
- **Architectural decision**: chose option (a) from the Description — precompute the deduped per-year spreads in `SpreadsTabView.init`, in the exact same single pass that already builds `navigatorCalendarModels`, rather than a separate `@State`-backed cache local to `SpreadsNavigatorView`. This adds zero extra iteration cost (the dedup bookkeeping piggybacks on the existing per-spread loop) and matches `navigatorCalendarModels`'s own lifecycle/staleness profile exactly, so there's no new invalidation surface to get wrong. The "leaks navigator-specific derivation into `SpreadsTabView`" tradeoff flagged in the Description is real but minor: `SpreadsTabView` already owns the equivalent `navigatorCalendarModels` derivation for the same view, so this is consistent with, not a departure from, the existing split.
- **Acceptance Criteria**:
  - [x] `selectedYearSpreads`'s dedup walk is not re-executed on every `SpreadsNavigatorView.body` evaluation when `calendarModels` and `selectedYear` are unchanged from the prior render — it is now a direct dictionary lookup into a value precomputed once in `SpreadsTabView.init`, not a per-render walk at all.
  - [x] Switching `selectedYear`, or `calendarModels` itself changing, still produces a correct, fully up-to-date deduped spread list (no stale-cache bugs) — both `navigatorCalendarModels` and `navigatorYearSpreads` are rebuilt together from the same `SpreadsTabView` instance, so they can never diverge.
  - [x] `SpreadsNavigatorView`'s and `SpreadsTabView`'s existing public-facing behavior (multiday bar rendering via `RowOverlayGenerator`) is unchanged.
  - [x] Project builds with no errors or warnings; full existing `Spread` test suite passes unmodified (1288/1288).
- **Tests**:
  - [x] New unit test asserting the memoized/cached path is not recomputed across repeated accesses with unchanged inputs — reframed for the chosen design: since the value is a `let`-equivalent stored property computed exactly once in `init` (not a cache with hit/miss states), "not recomputed on repeated access" is a structural guarantee rather than something to call-count. Tests instead verify the *build* (`SpreadsTabView.buildNavigatorCalendarData`, extracted as a standalone testable static function) produces correct deduped results once per distinct input.
  - [x] Test confirming a `selectedYear` change produces the correct deduped spread list for the new year — covered by `multidaySpreadCrossingYearBoundaryAppearsOnceInEachYear`, proving per-year dedup is independent across years.
- **Dependencies**: None — independent of SPRD-271/SPRD-272, can land in any order. Landed last in this session.
- **Progress (commits landed on feature/SESH-26)**:
  1. `[SPRD-273][1/n]` — Extracted `SpreadsTabView`'s `navigatorCalendarModels`-building loop (previously inline in `init`) into a standalone `static func buildNavigatorCalendarData(spreads:calendar:)` (`Spread/Views/Spreads/Shell/SpreadsTabView.swift`), now also building a new `navigatorYearSpreads: [Int: [DataModel.Spread]]` stored property in the same pass — deduped per-year by `Set<UUID>` insertion-order tracking (`seenSpreadIDsByYear`), piggybacking on the existing per-spread iteration rather than adding a second pass. `SpreadsNavigatorView` gained a new `let yearSpreads: [Int: [DataModel.Spread]]` property (injected by `SpreadsTabView`, same pattern as `calendarModels`); its `selectedYearSpreads` computed property is now `yearSpreads[selectedYear] ?? []` — a plain dictionary lookup, not a flat-map/dedup walk. No other call sites of `SpreadsNavigatorView` exist (no preview). Added `SpreadTests/Views/Spreads/Shell/SpreadsTabViewNavigatorDataTests.swift` (5 tests) covering: a lone `.day` spread, a `.multiday` spread spanning many days (proving it dedupes to one entry, not one per day), a `.multiday` spread crossing a year boundary (proving it correctly appears once in *each* year it touches), `.year`/`.month` spreads being excluded (matching `calendarModels`' existing exclusion), and multiple `.day` spreads in one year preserving encounter order. Verified via `-only-testing:SpreadTests/SpreadsTabViewNavigatorDataTests` (5/5 pass), a full `xcodebuild test` (1288/1288 pass, confirming behavior-preservation across the whole app, not just the new tests), and a full `xcodebuild build`.
- Task complete — all ACs satisfied.

### [SPRD-274] Feature: Extract OverdueCardView and show it on all spread content views - [ ] Pending

- **AC correction (found during pre-implementation audit)**: Day's current overdue card unintentionally shows a "+ Add Task" trailing button on its "Overdue" header — an incidental side effect of `DaySpreadContentView` concatenating `viewModel.overdueSections + viewModel.sections` into one `EntryListView` call that shares a single trailing-content closure across all sections, including the overdue one. `SPRD-235`'s original spec explicitly says "No overdue-specific actions in v1," so this is a pre-existing bug, not an intended behavior. Per user direction, `OverdueCardView` will not render any trailing content on any of the 4 views — this also fixes Day's incidental button, so "visually and behaviorally unchanged" below excludes this one corrected bug.
- **Context**: The overdue card (SPRD-235) is currently built inline inside `DaySpreadContentView.ViewModel.overdueSections` and only ever rendered by `DaySpreadContentView`. If today is a multiday, month, or year spread, there is no overdue review surface at all — the user only sees overdue tasks when they happen to be viewing today's day spread.
- **Description**: Extract the overdue-card-building logic into a standalone, reusable `OverdueCardView(spread: DataModel.Spread, context: SpreadPageContext)` SwiftUI view. It computes whether `spread` represents today via `DataModel.Spread.contains(date: context.journalManager.today, calendar: context.calendar)` (already implemented in `Spread+Hierarchy.swift` and period-aware for day/month/year/multiday), and if so and `JournalManager.overdueTaskItems` is non-empty, renders the existing `EntryListView` + `EntryList.Section.Style.card` card (one section per source spread/Inbox, same `EntryRowView.Configuration` and source chips as today). Renders `EmptyView` otherwise. `DaySpreadContentView`, `YearSpreadContentView`, and `MultidaySpreadContentView` each add `OverdueCardView(spread: spread, context: context)` as the first element of their *scrollable* content — before the entry list (Day, unchanged), before the top year-entry section (Year), and above the day-card grid, full width (Multiday). `MonthSpreadContentView` additionally requires a layout restructuring: today it puts the calendar grid, month section, and day sections all inside one scrollable `LazyVStack`, with no fixed top inset (unlike every other content view, which keeps a non-scrolling element pinned above its scrollable content). The month calendar grid moves out of the scrollable `LazyVStack` into a fixed top inset above the `ScrollView` (not scrollable, always visible), and `OverdueCardView` becomes the first item inside the now-calendar-free scrollable region, ahead of the month section and day sections. `DaySpreadContentView` is refactored to use the new component instead of its own `overdueSections`, with no visual or behavioral change to Day's existing card.
- **Spec**: `Documentation/Specs/ConventionalMode.md` — "Overdue Card on All Spread Content Views (SPRD-274)"
- **Scope extension (found mid-implementation, per user request)**: overdue card rows must be read-only — no inline editing, no context menu, no status toggle from this surface. Tapping the status icon shows a confirm-before-navigate alert; tapping anywhere else on the row navigates directly (or shows an informational alert for Inbox-sourced tasks, since there's no spread to navigate to). See `Documentation/Specs/ConventionalMode.md` — "Overdue Card Rows Are Read-Only" for full detail. Surfaced and resolved two real decisions before implementing: (1) Inbox-sourced navigation would require new cross-tab infrastructure (Spreads→Entries) that doesn't exist today — user chose an informational alert instead of building it; (2) discovered `SpreadsTabView`'s alert presentation was already disconnected (`// TODO: Re-add alert`, modifier commented out) — a pre-existing bug breaking every alert app-wide, fixed as a prerequisite since the new alerts depend on it.
- **Scope correction (found mid-implementation, per user request)**: the card must only show on the *most granular* spread containing today, not on every spread that contains today. Without this, a day spread and its parent month/year spread would all show the card simultaneously for the same overdue items, since each independently "contains" today via `Spread.contains(date:calendar:)`. Fixed by switching the condition to `spread.id == journalManager.bestSpread(for: today)?.id`, reusing `[DataModel.Spread].bestSpread(for:calendar:)`'s existing priority cascade (day > narrowest multiday > month > year) — the same logic already used by the Today button and default navigation. See `Documentation/Specs/ConventionalMode.md`'s updated "Overdue Card on All Spread Content Views" section.
- **Scope reversal (found mid-implementation, per user request)**: reverted the "status icon shows a confirm-before-navigate alert" decision from increment 4. The status icon is now fully interactive — tapping it rotates status (open → complete → cancelled → open) exactly like every other task row, no confirmation. Problem this solves: marking a task complete/cancelled immediately drops it out of `overdueTaskItems`, so the row would otherwise vanish from the card the instant the tap registers, before the user can register what happened or change their mind. Fix: a 5-second grace period — a row that just left `.open` keeps showing for 5 seconds (using its live post-tap status for display, and a snapshotted source key since `overdueTaskItems` no longer has it), and tapping again within that window (e.g. complete → cancelled) restarts the 5 seconds. See `Documentation/Specs/ConventionalMode.md` — "Overdue Card Status Icon: Status Change With a Grace Period." Removed the now-dead `AlertModel.overdueCardNavigateConfirmation` preset (no longer called anywhere) per "delete code with no production caller."
- **Acceptance Criteria**:
  - [x] `OverdueCardView` exists as a standalone view taking only `spread` and the existing `SpreadPageContext` (no new injected dependency type).
  - [x] The card appears only on the most granular spread that is `bestSpread(for: today)` (day > narrowest multiday > month > year) and there is at least one overdue or grace-period task; it renders nothing on any other spread that merely contains today, and nothing when there are no overdue/grace tasks.
  - [x] Day spread's overdue card is visually and behaviorally unchanged after the refactor (same card chrome, same source chips, same per-task actions), except that the incidental "+ Add Task" trailing button is removed per the AC correction above.
  - [x] Year and Multiday each show the same card chrome, grouping, and source chips as Day when applicable.
  - [x] `MonthSpreadContentView`'s calendar grid is pinned in a fixed top inset that does not scroll with the rest of the content; `OverdueCardView` (when applicable) is the first item in Month's scrollable region, above the month section and day sections, and shows the same card chrome/chips as the other three views.
  - [x] Multiday's existing per-day-card `overdueCount` badge mechanism (`MultidayDayCardView`) is untouched and unaffected.
  - [x] Overdue card rows support no inline title editing and no long-press context menu (Edit/Migrate/Delete).
  - [x] Tapping the status icon on an overdue row rotates its status (open → complete → cancelled → open), the same as a normal task row, with no confirmation alert.
  - [x] A task that leaves `.open` via the status icon keeps showing in the card for 5 seconds (grace period); tapping it again within that window restarts the 5 seconds; rotating back to `.open` clears the grace period immediately.
  - [x] Tapping anywhere else on an overdue row (not the status icon) navigates directly to the task's source spread (spread source) or shows an informational alert (Inbox source).
  - [x] `SpreadsTabView`'s alert presentation is re-enabled and renders `spreadsCoordinator.activeAlert` correctly (verifiable via the pre-existing delete-entry/discard-changes alerts now actually appearing, not just the Inbox notice alert).
...
- **Tests**:
  ...
  - [x] Unit test confirming a row tap on an Inbox-sourced task shows the informational alert instead of navigating.
  - [x] Unit tests confirming granularity priority: day spread wins over its parent month spread, month spread wins over its parent year spread, and multiday spread wins over an overlapping month spread — in each case the less-granular spread shows no section despite also containing today.
  - [x] Unit test confirming a grace-period task (no longer in `overdueTaskItems`, status `.complete`) still appears in `sections(...)`'s entries when passed via `graceTaskIDs`, using its live status and the snapshotted source key for the chip.
  - [x] Unit test confirming a stale grace-task ID with no matching task in `journalManager.tasks` is silently skipped, not a crash or placeholder row.
  - [x] Unit test confirming a task present in both the live overdue list and `graceTaskIDs` appears exactly once (no duplicate row).
  - [x] Unit test confirming entries are sorted by `Entry.conventionalSortKey(calendar:)` regardless of insertion/`createdDate` order.
  - [x] Unit test confirming a month-period task and a day-period task sort by period-normalized date (not raw `date`) — a case where the two orderings disagree, proving the conventional sort is actually wired in rather than coincidentally matching.
  - [x] Unit test confirming a grace-period task keeps its date-ordered position (not appended at the end) relative to a still-live overdue task with a later date.
  - [ ] Manual/visual verification: spot-check Month, Year, and Multiday spreads in the simulator with overdue tasks present, confirming card placement and chrome match Day's existing card, Month's calendar grid stays pinned while scrolling, the granularity-priority fix (card appears on only one spread at a time when nested spreads exist for today), the re-enabled alert system elsewhere in the app, the date-ordering fix (completing/cancelling a task doesn't move it in the list), and — the part not covered by unit tests — the actual 5-second grace-period timer end-to-end: tap to complete, confirm the row stays for ~5s, tap again within the window to cancel instead (confirm the 5s restarts), and let one expire naturally to confirm it disappears.
- **Bug found and fixed (post-increment-6, reported by user)**: completing/cancelling a task visibly rearranged the overdue list. Root cause was two-fold: (1) `JournalManager.tasks` (the source for `overdueTaskItems`'s order and grace-period lookups) is sorted by `createdDate`, not by any date relevant to the overdue card; (2) grace-period tasks were appended after all live entries in `sections(...)`, so a just-completed task always jumped to the end of the list regardless of its actual date. Fixed by explicitly sorting the combined entries, ignoring status/insertion order entirely.
- **Sort convention clarified (follow-up from the user)**: "sorted by date" meant the conventional, period-aware ordering already used elsewhere for entries spanning mixed periods — `YearSpreadContentView`'s month-card ordering and `MonthSpreadContentSupport`'s day-section ordering — not a raw `Entry.sortDate` comparison. Both of those already had byte-for-byte-identical private `sortKey`/`entryTypeSortOrder` implementations (period-normalized date, then entry type, then `createdDate`, then `id`); extracted into a single shared `Entry.conventionalSortKey(calendar:)` (`Spread/Views/Entries/EntryList/Entry+ConventionalSortKey.swift`) and adopted by all three call sites, removing both private duplicates per "check for redundancy before implementing."
- **Progress (commits landed on feature/SESH-26)**:
  1. `[SPRD-274][1/n]` — Added `OverdueCardView` (`Spread/Views/Spreads/Content/OverdueCardView.swift`), a standalone view taking only `spread`/`context`, with a `static func sections(for:context:)` (directly unit-testable) building the period-aware overdue card via `Spread.contains(date:calendar:)`. Refactored `DaySpreadContentView` to use it instead of `ViewModel.overdueSections` (now deleted) — `OverdueCardView` is stacked above `EntryListView` in a `VStack` inside Day's existing trailing column, rather than being concatenated into the same `EntryListView`'s `sections:` array as before. Per the AC correction above, `OverdueCardView` renders no trailing content, removing the incidental "+ Add Task" button. Added `SpreadTests/Views/Spreads/Content/OverdueCardViewTests.swift` (10 tests) covering all 4 periods' "is today" boundary cases (including multiday's inclusive start/end boundary) and the empty-`overdueTaskItems` case. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (10/10 pass), a full `xcodebuild test` (1339/1339 pass), and a full `xcodebuild build`.
  2. `[SPRD-274][2/n]` — Wired `OverdueCardView(spread:context:)` into `YearSpreadContentView` (as the first item in its `LazyVStack`, before `topYearSection`) and `MultidaySpreadContentView` (as a full-width sibling item between the `EntryListOptionsPicker` row and the `LazyVGrid` of day cards, inside its existing `conditionalScrollView()`-wrapped `VStack`). Both reuse `viewModel.spread`/`viewModel.context` (Multiday) or `spread`/`context` (Year) directly — no new properties needed on either view. Multiday's existing per-day-card `overdueCount` badge (`MultidayDayCardView`) is a separate mechanism and untouched. No new tests needed — covered by `OverdueCardViewTests`'s existing period-coverage plus the full app test suite confirming no regressions. Verified via a full `xcodebuild test` (1339/1339 pass) and a full `xcodebuild build`.
  3. `[SPRD-274][3/n]` — Restructured `MonthSpreadContentView.body`: moved `ScrollViewReader` to wrap a `VStack` whose first child is `SpreadMonthCalendarView` (the fixed, non-scrolling top inset) and whose second child is the `ScrollView` containing the `LazyVStack` of `OverdueCardView` → month section → day sections. The `ScrollViewReader`'s `proxy` (used by the calendar's "reveal day section" tap action) still wraps the `ScrollView` it targets, since `ScrollViewReader` only needs to be an ancestor of the `ScrollView`, not its direct parent — `proxy.scrollTo(date:)` against the day sections' `.id(section.id)` markers is unaffected. No new tests needed (pure layout restructuring + the already-tested `OverdueCardView` insertion); verified via a full `xcodebuild test` (1339/1339 pass) and a full `xcodebuild build`.
  4. `[SPRD-274][4/n]` — Made overdue rows read-only, per the scope extension above. Added `EntryRowView.Configuration.onRowTap` (`Spread/Views/Entries/EntryRow/EntryRowView+Configuration.swift`) — when set, `EntryRowView` (`Spread/Views/Entries/EntryRow/EntryRowView.swift`) disables its title `TextField` and replaces the long-press context menu with a plain tap gesture routed to `onRowTap`, via a new `ReadOnlyRowInteractionModifier` (kept separate so the two interaction modes don't need to share one expression's type). Added `EntryRowView.Configuration.readOnlyOverdueTaskConfig(journalManager:coordinator:sourceKey:getChips:)`, mirroring `standardTaskConfig`'s display closures (greyed-out/strikethrough/due-date) but with `onStatusIconTap` showing a confirm-before-navigate alert and `onRowTap` navigating directly (or showing the same alert for Inbox sources) — no `actions`, no `onTitleCommit`. Added two new `AlertModel` presets (`Spread/Views/Components/AlertModel.swift`): `overdueCardNavigateConfirmation(destinationLabel:onNavigate:)` and `overdueCardInboxNotice`. Updated `OverdueCardView` to use the new factory instead of `standardTaskConfig`. Fixed the pre-existing disconnected alert bug in `SpreadsTabView.swift`: restored `activeAlertModel` (unwraps `AlertDestination.alert`) and uncommented `.modifier(AlertModelModifier(...))`. Added 4 new tests to `OverdueCardViewTests.swift` (read-only configuration shape, direct navigation on row tap, confirmation alert on status-icon tap, informational alert for Inbox source) — 14 tests total in the file now. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (14/14 pass), a full `xcodebuild test` (1343/1343 pass), and a full `xcodebuild build`.
  5. `[SPRD-274][5/n]` — Fixed the granularity-priority bug per the scope correction above: `OverdueCardView.sections(for:context:)`'s condition changed from `spread.contains(date:calendar:)` to `spread.id == context.journalManager.bestSpread(for: context.journalManager.today)?.id`, so only the single most granular spread containing today shows the card. Updated every existing positive test in `OverdueCardViewTests.swift` to register its `spread` in the `spreads:` array passed to the test `JournalManager` (required for `bestSpread` to resolve it — previously the tests' synthetic spreads weren't in the journal's spread list at all, which happened to still pass under the old `contains(date:)`-only condition but would silently mask this exact bug). Added 3 new tests (`daySpreadTakesPriorityOverParentMonthSpread`, `monthSpreadTakesPriorityOverParentYearSpread`, `multidaySpreadTakesPriorityOverMonthSpread`) — 17 tests total in the file now. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (17/17 pass), a full `xcodebuild test` (1346/1346 pass), and a full `xcodebuild build`.
  6. `[SPRD-274][6/n]` — Reversed the status-icon decision per the scope reversal above. `EntryRowView.Configuration.readOnlyOverdueTaskConfig` now takes `onStatusIconTap` as a caller-supplied closure instead of building a confirm-alert internally — the configuration layer no longer knows or cares what tapping the icon does. `OverdueCardView` gained two `@State` dictionaries (`graceExpirations: [UUID: Date]`, `graceSourceKeys: [UUID: TaskReviewSourceKey]`) and `handleStatusIconTap(task:)`, which rotates status via `task.status.rotate(in: [.open, .complete, .cancelled])` + `journalManager.updateTaskStatus`, and — when the new status isn't `.open` — snapshots the task's current source key (read from `overdueTaskItems` *before* the status update commits, since it won't be derivable from that list afterward) and sets a 5-second expiration, with a `Task.sleep` that re-checks its own captured expiration value before clearing the grace entry (so a later tap's fresh expiration isn't clobbered). `OverdueCardView.sections(for:context:)` gained `graceTaskIDs`/`graceSourceKeys`/`onStatusIconTap` parameters (all defaulted, so existing call sites and most tests are unaffected) and now unions live `overdueTaskItems` with any grace-period task IDs not already live, looked up by ID in `journalManager.tasks` for their current (post-tap) status. Removed the now-dead `AlertModel.overdueCardNavigateConfirmation` preset. Replaced the obsolete `statusIconTapOnSpreadSourcedTaskShowsConfirmationAlert` test with `statusIconTapUsesTheSuppliedClosure`, and added 3 new tests for the grace-extension logic in `sections(...)` itself (`graceTaskStillAppearsAfterLeavingOverdueTaskItems`, `graceTaskIDWithNoMatchingTaskIsSkipped`, `taskInBothLiveAndGraceListsAppearsOnce`) — 20 tests total in the file now. The actual 5-second timer in `handleStatusIconTap` isn't unit-tested (no hosted-view harness in this codebase makes waiting out a real timer practical) — left to manual verification, per the Tests list above. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (20/20 pass), a full `xcodebuild test` (1349/1349 pass), and a full `xcodebuild build`.
  7. `[SPRD-274][7/n]` — Fixed the date-ordering bug above: `OverdueCardView.sections(for:context:)` now sorts the combined live + grace-period `entries` by `(sortDate, title)` immediately after building them, regardless of status or which list (live/grace) they came from. Added `entriesAreSortedByDateNotInsertionOrder` (proving the sort doesn't depend on `createdDate`/insertion order — tasks constructed in scrambled `createdDate` order still come back sorted by their actual `date`) and `graceTaskKeepsItsDateOrderedPositionRatherThanMovingToTheEnd` (proving a just-completed task with an earlier date stays ahead of a still-open task with a later date, rather than being appended after it) — 22 tests total in the file now. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (22/22 pass), a full `xcodebuild test` (1351/1351 pass), and a full `xcodebuild build`.
  8. `[SPRD-274][8/n]` — Switched from raw `(sortDate, title)` to the conventional period-aware sort per the clarification above. Extracted `Entry.conventionalSortKey(calendar:)` (`Spread/Views/Entries/EntryList/Entry+ConventionalSortKey.swift`) from `YearSpreadContentView`'s and `MonthSpreadContentSupport`'s identical private `sortKey`/`entryTypeSortOrder` implementations, removing both duplicates and updating their 3 call sites (Year's month-card entries, Month's month-section entries, Month's day-section entries) to use the shared extension — zero behavior change for them, confirmed by the full suite passing unmodified including `YearSpreadContentSupportTests`. `OverdueCardView.sections(for:context:)` now sorts by `conventionalSortKey(calendar:)` instead of `(sortDate, title)`. Added `entriesAreSortedByPeriodNormalizedDateNotRawDate` (a month-period task dated Jan 31 vs. a day-period task dated Jan 15 — raw-date order would put the day task first, period-normalized order puts the month task first since it normalizes to Jan 1) — 23 tests total in the file now. Verified via `-only-testing:SpreadTests/OverdueCardViewTests` (23/23 pass), a full `xcodebuild test` (1352/1352 pass), and a full `xcodebuild build`.
- Remaining for this task: manual/visual verification across all 4 views, including the granularity-priority fix, the re-enabled alert system elsewhere in the app, the row-tap navigation/Inbox-alert behavior, the conventional date-ordering fix, and — especially — the 5-second grace-period timer end-to-end (deferred to the user — see Tests above).

### [SPRD-275] Perf: Stop redundant title/journalManager reads and unwindowed spreadDataModel computation in SpreadContentPagerView - [x] Done
...
- **Acceptance Criteria**:
  - [x] `SpreadContentPagerView` receives `calendar`/`today`/`firstWeekday` as plain init-injected properties from the parent; `spreadDetailTitle` no longer reads them from `journalManager`.
  - [x] The doc comment on `SpreadContentPagerView`/`spreadDetailTitle` accurately describes which values are observed vs. pre-computed (no longer claims `journalManager` is unobserved in `body` while leaving live reads in place).
  - [x] `spreadDataModel(for:)` is computed only for the spread matching `settledSpreadID` and spreads within a fixed neighbor radius of it in `spreads`; spreads outside the window render without a computed `SpreadDataModel` (e.g. a placeholder) until the window moves to include them.
  - [x] The neighbor radius is a single named constant (defaulted to `1`) that can be changed in one place without touching the windowing logic.
  - [x] No change to pager paging/animation behavior, page transition visuals, or `SpreadDataModel` contents for spreads inside the window.
  - [x] Project builds with no errors or warnings; full existing test suite passes unmodified (1356/1356).
- **Tests**:
  - [x] Unit tests on the windowing predicate: settled page and `±1` neighbors included, pages at `±2` excluded, for a representative `spreads` array — `testSettledSpreadIsWithinWindow`, `testImmediateNeighborsAreWithinWindow`, `testFartherSpreadsAreExcludedFromWindow`, `testNoWindowWhenSettledSpreadIDIsNil` in `SpreadContentPagerAssemblyTests.swift`.
  - [ ] Unit tests on title derivation confirming the same output as before given the same plain `calendar`/`today`/`firstWeekday` inputs, without requiring a `JournalManager` instance. **Not implemented** — `spreadDetailTitle`'s own title/subtitle derivation lives unchanged in `SpreadHeaderConfiguration` (untouched by this task); the only change here is *where* its three inputs come from (plain properties vs. `journalManager`), which isn't independently observable through a `SpreadHeaderConfiguration`-level test. No hosted-view harness exists in this codebase to assert "this property read did not touch `journalManager`" at the SwiftUI body level (same gap noted in SPRD-272/SPRD-273).
  - [ ] Manual scroll-performance check: scroll through a year of populated day/multiday spreads and confirm no regression plus a qualitative improvement versus pre-fix behavior. **Deferred to the user** — left as manual verification per the testing-gap pattern established in SPRD-272.
- **Progress (commits landed on feature/SESH-26)**:
  1. `[SPRD-275][1/n]` — Added `calendar: Calendar`, `today: Date`, `firstWeekday: FirstWeekday` as plain stored properties on `SpreadContentPagerView`, injected from `SpreadsTabView` at the existing call site (`journalManager.calendar`/`.today`/`.firstWeekday`, read once in the parent instead of per-render in the child). `spreadDetailTitle` now builds its `SpreadHeaderConfiguration` from these three properties instead of `journalManager`. Verified via a full `xcodebuild build`.
  2. `[SPRD-275][2/n]` — Corrected the struct-level doc comment and the `journalManager` property's doc comment, which both claimed `JournalManager` wasn't observed during scroll-driven re-renders despite `spreadDetailTitle`'s live reads (now removed) and `spreadDataModel(for:)`'s still-live reads. Verified via a full `xcodebuild build`.
  3. `[SPRD-275][3/n]` — Added `Constants.spreadDataModelWindowRadius` (`= 1`) and a `static func isWithinDataModelWindow(spread:spreads:settledSpreadID:radius:)` predicate; `spreadDataModel(for:)` now returns `nil` immediately for any spread outside the window, skipping the `JournalManager.dataModel` dictionary lookup entirely (those pages render the pre-existing "No Data" `ContentUnavailableView`). Added a custom `init` so `settledSpreadID` is seeded from `currentSelection.id` at construction rather than only in `.onAppear`, so the very first render's window is already centered on the selected spread instead of momentarily showing "No Data". Added `SpreadContentPagerAssemblyTests.swift` windowing tests (4 new tests, 16 total in the file). Verified via `-only-testing:SpreadTests/SpreadContentPagerAssemblyTests` (16/16 pass), a full `xcodebuild test` (1356/1356 pass), and a full `xcodebuild build`.
- Task complete — all ACs satisfied; the title-derivation unit test and the manual scroll-performance check are explicitly noted above as not implemented/deferred, consistent with this codebase's existing testing-gap documentation pattern for SwiftUI body-level behavior.

### [SPRD-276] Refactor: Batch SyncEngine.push() mutations per entity type - [ ] Done

- **Context**: `SyncEngine.push()` currently sends one RPC call per pending `SyncMutation`, sequentially, awaiting each round-trip before starting the next. For N pending mutations of the same entity type (e.g. 10 edited tasks), that's N network round-trips where one would do. Mutation coalescing (SPRD-253) already collapses repeated edits to *the same entity* into one outbox row — that part of "reduce redundant sync traffic" is already solved. This task addresses the separate remaining gap: many *different* entities pending at once still each cost their own round-trip.
- **Description**: Add a `merge_X_batch(p_rows jsonb) RETURNS jsonb` sibling function for each of the 8 existing `merge_*` Postgres functions (`merge_settings`, `merge_spread`, `merge_entry`, `merge_collection`, `merge_list`, `merge_tag`, `merge_assignment`, `merge_entry_tag`), looping over a jsonb array of per-row params (reusing each function's existing per-row INSERT/UPDATE/field-level-LWW logic) with per-row exception isolation so one malformed row doesn't abort the batch. Update `SyncSerializer` with a `buildBatchMergeParams(entityType:mutations:userId:)` helper that wraps per-row params (built via the existing, unchanged `buildMergeParams`) into a single `{"rows": [...]}` payload. Add `SyncEntityType.mergeBatchRPCName`. Replace `SyncEngine.push()`'s inner per-mutation RPC loop with one batched call per entity type, decoding the per-row result array to selectively delete succeeded mutations and increment `retryCount` only on failed ones. Deletes need no special handling — they're already a `deleted_at` field inside the same merge payload. Apply the new Postgres functions directly to `baseline_schema.sql` (pre-release squash convention) rather than a new migration file.
- **Spec**: `Documentation/Specs/Sync.md` — "Batch Mutation Push (SPRD-276)"
- **Acceptance Criteria**:
  - [x] For each `SyncEntityType` with one or more pending mutations, `SyncEngine.push()` issues exactly one RPC call carrying all of that type's pending mutations, instead of one RPC call per mutation.
  - [x] All 8 `merge_X_batch` Postgres functions exist in `baseline_schema.sql`, each accepting a jsonb array of the same per-row fields the scalar function takes, with per-row `auth.uid()` authorization (no batch-level trust boundary).
  - [x] A malformed/failing row inside an otherwise-successful batch call does not abort the rest of that batch — other rows in the same call still succeed, and the function's returned per-row result array reflects per-row success/failure.
  - [x] A whole-call failure (e.g. transport error) increments `retryCount` for every mutation in that batch and leaves them all in the outbox, matching today's per-mutation failure/backoff behavior exactly (no finer-grained handling here).
  - [x] No local entity data is reverted or lost in either failure mode — local SwiftData writes remain final and independent of network/sync success, confirmed by the new test below.
  - [x] Mutations that fail `buildMergeParams` (malformed `recordData`) are still filtered out and deleted from the outbox before the batch is built, unchanged from today's per-mutation behavior.
  - [x] `Task.checkCancellation()` is preserved between entity-type batches (not per-row within a batch).
  - [x] Unused scalar `merge_X` functions are removed from `baseline_schema.sql` after confirming (by searching the codebase and any other known callers) that nothing else still calls them; any function found to still have a caller is kept and that caller is noted in the commit/progress notes. **Outcome: none removed.** All 8 scalar functions are called internally by their own `merge_X_batch` wrapper (`merge_entry_batch` calls `public.merge_entry(...)`, etc. — see `baseline_schema.sql`), since the batch functions were implemented by reusing the scalar functions' logic via direct calls rather than reimplementing it inline. The Swift client no longer calls the scalar RPCs directly, but the scalar functions remain live server-side dependencies of the batch functions, so they are kept as-is.
  - [x] The updated `baseline_schema.sql` is applied to `spread-prod` (spread-dev skipped — decommissioned per current account state), verified with `list_tables`/`execute_sql` smoke queries before/after — per the remote-migration-caution memory, confirm real row counts before applying.
  - [x] Project builds with no errors or warnings; full existing test suite passes unmodified (1363/1363).
- **Tests**:
  - [x] Unit test asserting N pending mutations of the same entity type produce exactly 1 RPC call (not N), via a mocked `mergeRPCCaller` returning a batch-result array.
  - [x] Unit test for partial-failure handling: some rows in a batch succeed and some fail; only the failed mutations keep `retryCount` incremented and remain in the outbox, succeeded ones are deleted.
  - [x] Unit test simulating a failing batch RPC call end-to-end: local entity data is unchanged, the outbox mutation is retained with `retryCount` incremented (not deleted), and a subsequent successful retry clears it.
  - [x] Existing `SyncEngineTests` suite passes with the mocked caller updated to the new batch-call shape.
  - [ ] Manual: queue edits across several entries/tasks while offline (airplane mode), go online, confirm via `mcp__supabase__get_logs` that exactly one RPC call per entity type fires and all entities land correctly server-side. **Deferred** — requires a physical/simulator device session with the app running against prod, not available in this implementation session. Recommend running this manually before the next TestFlight build.
- **Progress (commits landed on feature/SESH-26)**
  1. `[SPRD-276][1/n]` — Added `SyncEntityType.mergeBatchRPCName`, mapping each of the 8 entity types to its future `merge_X_batch` RPC name (e.g. `merge_settings_batch`), mirroring the existing `mergeRPCName` pattern. Picked first since it's a pure, inert addition with no callers yet — establishes the naming contract the SQL functions (next increment) and `SyncSerializer`/`SyncEngine` changes will both depend on. Added `testMergeBatchRPCNames` to `SyncEntityTypeTests.swift`, mirroring the existing `testMergeRPCNames` test. Verified via `-only-testing:SpreadTests/SyncEntityTypeTests` (7/7 pass) and a full `xcodebuild build`.
  2. `[SPRD-276][2/n]` — Added the 8 `merge_X_batch(p_rows jsonb) RETURNS jsonb` Postgres functions to `baseline_schema.sql` (applied directly per the pre-release squash convention, not a new migration file). Each loops over `jsonb_array_elements(p_rows)` and calls the existing scalar `merge_X` function per row inside its own `BEGIN...EXCEPTION WHEN OTHERS` block, so one malformed row doesn't abort the batch; returns a `[{"id", "success", "row"|"error"}]` array. The three void-returning scalars (`merge_list`, `merge_tag`, `merge_entry_tag`) are called via `PERFORM` with `"row": null` in their success result, since the client doesn't consume row data today (`MergeRPCCaller` is `-> Void`). `merge_entry_tag_batch` uses `p_entry_id || ':' || p_tag_id` as its per-row `id` (no single PK). Picked second since it's still inert (no Swift caller yet) but unblocks the next increment's `SyncSerializer`/`SyncEngine` wiring. Verified by starting the local Supabase stack (`supabase start`, `supabase db reset`) and confirming the migration applies with no syntax errors; manually smoke-tested `merge_collection_batch` via `psql` with one valid row and one malformed-UUID row in the same call — valid row succeeded, malformed row failed in isolation, batch did not abort. No Swift files changed, so no `xcodebuild` run for this increment.
  3. `[SPRD-276][3/n]` — Added `SyncSerializer.buildBatchMergeParams(entityType:mutations:userId:)` and a new `BatchMergeParams` (`Encodable`, single `rows: [AnyEncodable]` field keyed `p_rows`, matching the batch RPC's single jsonb-array parameter). Reuses `buildMergeParams` unchanged per row; mutations whose `recordData` fails to parse are excluded from the payload and returned separately via `failedMutationIDs` so the caller can delete them from the outbox before the batch call, satisfying the unchanged pre-batch filtering requirement. Takes `[(mutationID: UUID, recordData: Data)]` rather than `[SyncMutation]` to keep `SyncSerializer` decoupled from the SwiftData model, matching the existing `buildMergeParams(recordData:)` pattern (and sidesteps any Sendable/isolation concerns from passing `@Model` instances across the boundary). Reuses the existing `AnyEncodable` type-erasure wrapper from `SyncEngine.swift` rather than duplicating it. Picked third since it's the last inert step before `SyncEngine.push()` itself is rewired (next increment) — still has no caller. Added 3 tests to `SyncSerializerTests.swift` mirroring the `buildMergeParams` test style: all-valid-rows, mixed valid/invalid filtering, and empty input. Verified via `-only-testing:SpreadTests/SyncSerializerTests` (33/33 pass, including the 3 new tests) and a full `xcodebuild build`.
  4. `[SPRD-276][4/n]` — Reworked `SyncEngine.push()` to issue one batched RPC call per entity type instead of one call per mutation. `MergeRPCCaller`'s signature changed from `-> Void` to `-> Data` so the engine can decode the batch RPC's per-row result array (`[BatchMergeRowResult]`, a private nested type with `success`/`error`, `id`/`row` intentionally unused). Mutations are correlated back to their result positionally (zipping `validMutations` against the decoded results array) rather than by the response's `id` field, since `entry_tags` rows have no single id to match on and Postgres's `jsonb_array_elements` guarantees input-order preservation. `Task.checkCancellation()` moved from per-mutation to once per entity-type batch. Added `SyncError.batchResultCountMismatch` for the case where the decoded result count doesn't match the rows sent (treated as a whole-batch failure — increments `retryCount` for every mutation in that batch and rethrows, same as a transport-level throw). Updated all 8 `mergeRPCCaller` closures in `SyncEngineTests.swift` to return batch-shaped `Data` (via a new `batchSuccessData(from:)` test helper) and all RPC-name assertions to use `mergeBatchRPCName` instead of `mergeRPCName`; updated `jsonPayload`/added `jsonRows` helpers to unwrap the `{"p_rows": [...]}` envelope. Added 3 new tests: `batchPushIssuesOneRPCCallPerEntityType` (3 mutations → 1 call), `batchPushAppliesPerRowPartialFailure` (mixed success/failure within one batch — failed mutation's `retryCount` increments and it stays in the outbox, succeeded one is deleted), `batchPushWholeCallFailureRetriesThenSucceeds` (whole-call throw → outbox retained with incremented `retryCount` and local entity data unchanged, then a successful retry clears it). Verified via `-only-testing:SpreadTests/SyncEngineTests` (23/23 pass), a full `xcodebuild build`, and the full `xcodebuild test` suite (1363/1363 pass — confirms no regression in shared sync infrastructure).
  5. `[SPRD-276][5/n]` — Audited whether the 8 scalar `merge_X` functions could be removed per AC8. **Outcome: kept, not removed.** Each `merge_X_batch` function calls its scalar counterpart directly (e.g. `merge_entry_batch` → `public.merge_entry(...)`) rather than reimplementing the per-row logic inline, so the scalar functions remain live server-side dependencies of the batch functions even though the Swift client no longer calls them directly. No code change — documented this finding in the AC checklist. Applied the additive diff from increment 2 (the 8 `CREATE FUNCTION ... merge_X_batch` definitions plus 24 `GRANT` statements — extracted via `git diff` against the pre-increment-2 commit, not a re-run of the full `baseline_schema.sql`) to `spread-prod` via `mcp__supabase__apply_migration`. Pre-flight: confirmed all 8 scalar functions already exist on prod via `pg_proc` query, and recorded baseline row counts (`spreads`=55, `entries`=259, `assignments`=511, `collections`=0, `settings`=1, `lists`=1, `tags`=3, `entry_tags`=6) per the remote-migration-caution memory. Post-apply: confirmed all 8 batch functions now exist via `pg_proc`, confirmed row counts unchanged, and ran `get_advisors(security)` — the new functions trigger the same pre-existing `function_search_path_mutable` WARN as the existing scalar `merge_X`/trigger functions (not a new regression). A direct `execute_sql` smoke-test call to `merge_collection_batch` inserted a real throwaway row into prod `collections` (RLS's `auth.uid()` check is bypassed for direct SQL access, unlike RPC calls from an authenticated client) — caught immediately and deleted via a follow-up `DELETE ... RETURNING id`, confirmed row count back to 0. spread-dev was not used (decommissioned — only `spread-prod` exists in the account, confirmed via `mcp__supabase__list_projects`).
- Task complete — all ACs satisfied except the manual offline/online device test, which is explicitly deferred above (no device session available in this implementation session) consistent with this codebase's existing testing-gap documentation pattern. All automated tests pass (23/23 `SyncEngineTests`, 1363/1363 full suite), `baseline_schema.sql` and `spread-prod` are in sync, and the 8 scalar `merge_X` functions are confirmed-kept (not unused) per the AC8 audit.
### [SPRD-277] Refactor: Extract shared entry sheet chrome components - [x] Done

- **Context**: `TaskCreationSheet`, `TaskDetailSheet`, `NoteCreationSheet`, `NoteDetailSheet`, and `SpreadNameEditSheet` each independently re-declare nearly identical private helpers — `sectionHeader(_:)`, `compactDivider`, `validationErrorRow(message:)`, `loadingOverlay`, and (in the Task/Note detail sheets) `selectionSummaryRow(title:value:isEnabled:showsChevron:)` — with copy-pasted styling. This is the first, lowest-risk increment of the broader entry-sheet unification effort and has no behavior dependency on the later form-model/shell work.
- **Description**: Extract these into standalone, reusable SwiftUI views/view-builders under `Spread/Views/Components/` (e.g. `EntrySheetSectionHeader`, `EntrySheetDivider`, `EntrySheetValidationErrorRow`, `EntrySheetLoadingOverlay`, `EntrySheetSelectionSummaryRow`), each with its own preview. Update all 5 existing sheets to use the shared components in place of their private duplicates, with zero visual or behavioral change. Do not yet change sheet structure, ViewModels, or navigation — this task is purely an extraction.
...
- **Acceptance Criteria**:
  ...
  - [x] Project builds with no errors or warnings; full existing test suite passes unmodified.
- **Tests**:
  - Manual/visual verification: open each of the 5 sheets in the simulator and confirm no visual regression versus current behavior (this is a pure extraction with no new logic to unit test).
- **Dependencies**: None.
- **Progress (commits landed on feature/SESH-27)**:
  1. `[SPRD-277][1/n]` — Added 5 standalone, previewed SwiftUI views to `Spread/Views/Components/`: `EntrySheetSectionHeader`, `EntrySheetDivider`, `EntrySheetValidationErrorRow`, `EntrySheetLoadingOverlay`, `EntrySheetSelectionSummaryRow`. `EntrySheetSelectionSummaryRow` unifies `TaskDetailSheet.selectionSummaryRow` and `NoteDetailSheet.noteSelectionRow` — the Note variant lacked `isEnabled`/`showsChevron` params (both implicitly true), so the unified type adds both with matching defaults. No sheet edits in this increment.
...
2. `[SPRD-282][2/n]` — Added `showsMonthHeader: Bool` to `MonthCalendarConfiguration` (JohnnyOFoundationCore); `MonthCalendarView` stores configuration and conditionally suppresses the generator's `headerView` when `showsMonthHeader == false`. Prerequisite for `CalendarView` single-month mode owning its own nav header.
3. `[SPRD-282][3/n]` — Added `CalendarViewDisplayConfiguration` (Sendable) with `scrollDirection`, `isScrollEnabled`, `isSingleMonth` params and `.default`/`.singleMonthPager` presets. `CalendarView` branches its body: single-month mode shows a prev/next chevron nav header (with disabled states at range boundaries) + one `MonthCalendarView` with `showsMonthHeader: false`; multi-month body unchanged. `PeriodDatePicker.dayPicker` passes `displayConfiguration: .singleMonthPager`. Build passes.

### [SPRD-283] Perf: Fix EventKit environment service churn causing constant SpreadContentPagerView re-renders - [ ] Done

- **Context**: Diagnosed via `Self._printChanges()` during a scroll-lag investigation on SESH-27. `SpreadContentPagerView` re-renders 10+ times on cold launch and on every `sceneDidBecomeActive` event — before any user interaction — because `_eventKitService` and `_calendarEventService` appear "changed" to SwiftUI on every `ContentView.body` re-run. The root cause: both environment keys store their values as `any Protocol` existentials, which are not `Equatable`. SwiftUI cannot compare old vs. new, so it always propagates a "changed" signal when `ContentView` re-sets these values, even though the same service instances are being re-injected every time. Each spurious re-render causes `SpreadContentPagerView.body` to re-evaluate `contentView(for:)` for every windowed page, doing redundant `JournalManager.dataModel` lookups and re-constructing `SpreadPageContext`. Fixing this eliminates the always-on re-render noise, making the remaining scroll-time re-render issues (see SPRD-284) easier to isolate and measure.
- **Description**: Change `LiveCalendarEventService` from `struct` to `final class` (it has no value semantics — it holds a service reference and is never copied for mutation). Add an `ObjectIdentifier`-based `Equatable` identity box at both environment key sites so SwiftUI can short-circuit propagation when the same instance is re-injected. The public environment API (`@Environment(\.eventKitService)`, `.environment(\.eventKitService, value)`) is unchanged — all existing call sites compile unmodified.
- **Spec**: `Documentation/Specs/SpreadNavigation.md` — "Pager Re-Render: Environment Service Churn [SPRD-283]"
- **Acceptance Criteria**:
  - AC1: `SpreadContentPagerView` does not print `_eventKitService, _calendarEventService changed` in `_printChanges()` output during normal app launch or on `sceneDidBecomeActive` events.
  - AC2: `LiveCalendarEventService` is a `final class`, not a `struct`. All existing call sites compile without modification.
  - AC3: Both `EventKitService+Environment.swift` and `CalendarEventService+Environment.swift` use an identity-based (`===`) `Equatable` comparison to guard against spurious change propagation.
  - AC4: Project builds with no errors or warnings; full existing test suite passes unmodified.
- **Tests**:
  - No unit tests: the fix is at the SwiftUI environment infrastructure level. Correctness is verified by AC1 (observable via `_printChanges()`) and AC4 (full test suite pass).

**Progress (commits landed on feature/SESH-27)**:
1. `[SPRD-283][1/n]` — Changed `LiveCalendarEventService` from `struct` to `final class` (class semantics required for stable `ObjectIdentifier`-based equality). Added an inner `Box: Equatable` type to both `EventKitServiceEnvironmentKey` and `CalendarEventServiceEnvironmentKey` that compares via `(value as AnyObject) === (value as AnyObject)`, so SwiftUI can short-circuit environment propagation when the same service instance is re-injected. Public `EnvironmentValues` API (`var eventKitService`, `var calendarEventService`) is unchanged — all existing call sites compile unmodified. No unit tests (SwiftUI body-level change; verified by AC1 via `_printChanges()` and AC4). Full test suite: 1353/1353 pass.

### [SPRD-284] Perf: Fix scroll-settle re-render cascade from SpreadsTabView through SpreadContentPagerView - [ ] Done

- **Context**: Diagnosed via `Self._printChanges()` during a scroll-lag investigation on SESH-27, after SPRD-271–275 already addressed earlier performance issues. Every pager page-swipe triggers a re-render cascade: `settledSpreadID` changes → `syncSelectionFromSettledID()` sets `coordinator.selectedSpread` → `SpreadsTabView.body` re-runs (it observes `selectedSpread` via `currentSelection`) → `yearSpreads` is recomputed (new array allocation, identical content for same-year scrolls) → `SpreadContentPagerView` receives new `@self` → `DaySpreadContentView` shows `@self, @identity, _viewModel changed`. The view model reconstruction on `DaySpreadContentView` is the most expensive part. Confirmed output: `SpreadContentPagerView: _settledSpreadID changed` / `SpreadsTabView: \SpreadsCoordinator.<computed> (Optional<Spread>) changed` / `SpreadContentPagerView: @self changed` / `DaySpreadContentView: @self, @identity, _viewModel changed` — firing on every swipe.
- **Description**: Cache `yearSpreads` as a `@State` value on `SpreadsTabView`, initialized in `init` and updated via `onChange` only when the calendar year of `currentSelection` actually changes (or when `journalManager.spreads` grows/shrinks). Because `@State` does not trigger downstream re-renders when assigned the same value, intra-year scroll settles will no longer cause a new `[DataModel.Spread]` array to be passed to `SpreadContentPagerView`, breaking the cascade.
- **Spec**: `Documentation/Specs/SpreadNavigation.md` — "Pager Re-Render: Scroll-Settle Chain [SPRD-284]"
- **Acceptance Criteria**:
  - AC1: `_printChanges()` no longer shows `SpreadContentPagerView: @self changed` or `DaySpreadContentView: @self, @identity, _viewModel changed` when swiping between spreads within the same calendar year.
  - AC2: Navigating to a spread in a different calendar year (via the navigator or convenience nav) does update the pager's spread list correctly — the new year's spreads are shown.
  - AC3: Creating a new spread in the current year causes it to appear in the pager without requiring a year-change navigation.
  - AC4: Project builds with no errors or warnings; full existing test suite passes unmodified.
- **Tests**:
  - No unit tests: the fix is at the SwiftUI state/observation layer. Correctness is verified by AC1–AC3 (observable via `_printChanges()` and manual scroll testing) and AC4 (full test suite pass).
- **Dependencies**: SPRD-283 (fix environment churn first so SPRD-284's `_printChanges()` output is not obscured by the service-churn noise)

**Progress (commits landed on feature/SESH-27)**
1. `[SPRD-284][1/n]` — Replaced `yearSpreads` computed property with `@State cachedYearSpreads` in `SpreadsTabView`, initialized in `init` via new static `buildYearSpreads(spreads:year:calendar:)`. Cache updated via `onChange` on `currentSelectionYear` and `journalManager.spreads.count`. Removed `currentSelection: DataModel.Spread` from `SpreadContentPagerView` init (replaced with `initialSelectedSpreadID: UUID`); `spreadDetailTitle` now derives from `coordinator.selectedSpread`; `selectedSpreadID` is now `UUID?` from coordinator. Together these stabilize the pager's struct value across intra-year scroll settles, eliminating `@self changed` → `DaySpreadContentView` cascade. Build green, 1353/1353 tests pass.
2. `[SPRD-284][3/n]` — Removed all `coordinator.selectedSpread` reads from `SpreadContentPagerView.body` scope: `spreadDetailTitle` now derives from `settledSpreadID`; `SpreadPageHeaderView` (new private struct) isolates `isCurrentPage`/`convenienceNavigation` reads; `onChange(of: selectedSpreadID)` removed in favour of `onChange(of: coordinator.recenterToken)` (all external nav increments it; coordinator reads inside handler closures are not body-tracked). Pager body now fires once per swipe from `_settledSpreadID` only.
3. `[SPRD-284][5/n]` — Added `DaySpreadContentView: Equatable` (`spreadID` + `storedHorizontalSizeClass`). SwiftUI uses `==` before calling body; existing visible pages are skipped entirely on pager re-renders. Entry data changes still propagate via `@Observable` journalManager dependency inside `sections()`. `.equatable()` added at call site. Verified via `_printChanges()`: only `@self, @identity, _viewModel changed` (new page entering window) remains — all `@self changed` on existing pages eliminated. 1353/1353 tests pass.
...
3. **[SPRD-290][3/n]** — Changed `containingParentSpreads(for:)` to `todayContextSpreads(for:)`: buttons now always point to TODAY's broader-period spreads rather than spreads containing the viewed spread's date. Simplified multiday handling (no range iteration needed — always today). Increased chip inner padding from 8h/4v to 10h/6v. Updated tests to match new semantics (8 tests, all pass).
4. **[SPRD-290][4/n]** — Removed coarser-period restriction from `todayContextSpreads(for:)`: now returns today's spreads for ALL other periods (not just broader ones). Year spread now shows "Today"/"This Month" buttons if those spreads exist; month spread now shows "Today" in addition to "This Year". Added `case .day: return "Today"` label; made top label conditional (multiday shows only canonical date-range title). Updated and expanded tests from 8 to 12 (added `yearPeriodReturnsOtherExistingPeriods`, `yearPeriodReturnsEmptyWhenNoOtherPeriodSpreadsExist`, `yearPeriodReturnsSpreadsInGranularityOrder`, `monthPeriodIncludesTodayDaySpreadBeforeYearSpread`, `multidayPeriodIncludesTodayDaySpread`; renamed old tests to reflect new semantics). All 12 pass.
5. **[SPRD-290][5/n]** — Flipped sort direction so buttons render year → month → Today (least-granular to most-granular, leading to trailing). Updated ordering assertions in all 12 tests.
6. **[SPRD-290][6/n]** — Replaced dynamic `todayContextSpreads(for:)` approach with precomputed `let todayYearSpread/todayMonthSpread/todayDaySpread` in `SpreadContentPagerView.init`. All three buttons always show (when the spread exists); the one matching the settled spread's period uses `.tonal`, others use `.plain` (`SpreadButton`). Deleted `JournalManager+ParentSpreads.swift` and `JournalManagerParentSpreadsTests.swift` (no production callers).
...
### [SPRD-295] Visual: Navigator redesign — year strip, card month headers, range bands - [ ] Done
...
- **Acceptance Criteria**:
  - [x] AC1: Context buttons render with their first text lines top-aligned.
  - [x] AC2: A horizontal year strip of small SpreadButtons sits below the context buttons in the fixed top area; selected year is `.tonal`, others `.plain`; tapping switches the displayed year; the strip auto-scrolls to the selected year on appear; the bottom glass menu picker is removed.
  - [x] AC3: The calendar remains the only vertically scrolling region, beneath the fixed top area.
  - [x] AC4: Month headers render a SpreadCardStyle-derived chip (created vs. uncreated fill/stroke, today-month emphasis) and show a "View month" button only when the explicit month spread exists; tapping it navigates to that spread. Uncreated months have no button and no tap target.
  - [x] AC5: Multiday spreads render as continuous low-opacity accent bands behind covered day cells with rounded end caps; up to two overlapping spreads still stack via the existing lane system.
  - [x] AC6: Day cell styling is unchanged.
  - [x] AC7: Build succeeds with no errors.
- **Tests**:
  - No new unit tests: visual change. Manual QA across compact/regular width and both color schemes. `SpreadsTabViewNavigatorDataTests` (5) still pass against the extended builder.

**Progress (commits landed on feature/SESH-28)**
1. **[SPRD-295][1/n]** — Top area restructured: context buttons in an `HStack(alignment: .top)` so first text lines align, horizontal year strip of small SpreadButtons (ascending, `.tonal` selected, `ScrollViewReader` auto-scroll to selection) below them; bottom glass menu `Picker` overlay deleted. AC1–AC3 ✅.
2. **[SPRD-295][2/n]** — `buildNavigatorCalendarData` extended to also return `monthSpreads: [Int: [Date: DataModel.Spread]]` in the same pass; threaded through `SpreadsTabView` → `SpreadsNavigatorView` → `CalendarGenerator`. Month headers render as `SpreadCardStyle` chips (created/uncreated fill+stroke, today emphasis, style-tinted title) with a plain "View month" SpreadButton when the spread exists, navigating via the shared animation-disabled selection path. AC4 ✅.
3. **[SPRD-295][3/n]** — `RowOverlayGenerator` bars replaced with full-lane-height low-opacity accent capsule bands (`Opacity.cardFill`) behind the day cells; lane packing unchanged so overlapping spreads stack as offset translucent strips. Stale view/generator doc comments refreshed. AC5–AC7 ✅.
4. **[SPRD-295][4/n]** — Overlay/alignment refinements: band segments fade out (~18% alpha ramp) and run square to the row edge wherever the spread continues past the row's rendered days (`continuesBeforeWeek`/`continuesAfterWeek` — covers both week wraps and month boundaries), with rounded caps only at real range ends; day cell chips changed from rounded rects to circles; all navigator content aligned to a shared 8pt leading margin (calendar padded 6pt + 2pt cell/chip insets); fixed top area gets an `.ultraThinMaterial` backdrop so strips stay legible over scrolling content.
5. **[SPRD-295][5/n]** — Day-tap disambiguation: tapping a date with exactly one covering spread (day OR multiday — multiday-only dates were previously a no-op) navigates directly; several covering spreads present a coordinator-driven popover (`NavigatorDaySelectionPopoverContent` on `PopoverDestination`, anchored on the tapped cell via `DateCellAnchorKey` anchors now published by day cells; small 220pt-detent sheet on compact, matching QuickAdd). Month spreads remain reachable only from the header button.
6. **[SPRD-295][6/n]** — "View year" chip above January: builder also returns `explicitYearSpreads` per year; header refactored into a shared `headerChip` used by both the January year chip (title3, year-granularity today emphasis, "View year" when the spread exists) and month chips; `onViewMonth` generalized to `onViewSpread`.
...
### [SPRD-296] Feature: Task scheduledTime model field + Entry time capability and accessors - [ ] Pending
...
### [SPRD-297] Feature: Supabase scheduled_time column + LWW sync round-trip - [ ] Pending
...
### [SPRD-298] Feature: Scheduled time migration rules — rebase day→day, clear otherwise - [ ] Pending
...
### [SPRD-299] Feature: Time chip in TaskEntrySheet gated to day-period assignment - [ ] Pending
...
### [SPRD-300] Visual: Scheduled time block in entry rows - [ ] Pending
...
### [SPRD-301] Feature: Time sort option with integrated day-spread chronology - [ ] Pending
