...
- Remaining for this task: tighten `OverlayPayload` to `Hashable & Sendable` across the row-overlay overlay/context/generator types, then add the corresponding cache to `MonthCalendarRowOverlayLayoutBuilder.makeWeekLayouts` with its own cache-hit/miss tests.
