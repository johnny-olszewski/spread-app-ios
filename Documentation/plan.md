...
- **Tests**:
  ...
  - [x] Unit test confirming entries are sorted by `(sortDate, title)` regardless of insertion/`createdDate` order.
...
- Remaining for this task: manual/visual verification across all 4 views, including the granularity-priority fix, the re-enabled alert system elsewhere in the app, the row-tap navigation/Inbox-alert behavior, the date-ordering fix, and — especially — the 5-second grace-period timer end-to-end (deferred to the user — see Tests above).
