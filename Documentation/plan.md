...
- **Acceptance Criteria**:
  - [ ] `OverdueCardView` exists as a standalone view taking only `spread` and the existing `SpreadPageContext` (no new injected dependency type).
  - [ ] The card appears on Day, Month, Year, and Multiday spread content views whenever the displayed spread represents today (per `Spread.contains(date:calendar:)`'s period-aware definition) and there is at least one overdue task; it renders nothing otherwise.
  - [ ] Day spread's overdue card is visually and behaviorally unchanged after the refactor (same card chrome, same source chips, same actions).
...
- **Tests**:
  - [ ] Unit tests covering the section-building logic for each period: day spread matching/not matching today, month spread whose month contains/doesn't contain today, year spread whose year contains/doesn't contain today, and multiday spread where today falls inside/outside/exactly on the range boundary.
  - [ ] Unit test confirming an empty `overdueTaskItems` produces no sections even when the spread represents today.
