...
- **Acceptance Criteria**:
  ...
  - [x] The card appears on Day, Month, Year, and Multiday spread content views whenever the displayed spread represents today (per `Spread.contains(date:calendar:)`'s period-aware definition) and there is at least one overdue task; it renders nothing otherwise. — Day done; Month/Year/Multiday wiring pending.
...
- **Tests**:
  ...
  - [ ] Manual/visual verification: spot-check Month, Year, and Multiday spreads in the simulator with overdue tasks present, confirming card placement and chrome match Day's existing card, Month's calendar grid stays pinned while scrolling, and the new tap/alert/navigate behavior works end-to-end (including confirming the now-re-enabled delete/discard-changes alerts elsewhere in the app still present correctly).
- **Progress (commits landed on feature/SESH-26)**:
...
- Remaining for this task: manual/visual verification across all 4 views, including the new tap/alert/navigate behavior and the re-enabled alert system elsewhere in the app (deferred to the user — see Tests above).
