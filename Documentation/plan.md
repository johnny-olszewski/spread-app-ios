...
### [SPRD-269] Feature: Replace SF Symbols with Phosphor icons via a new SpreadTheme.Icon namespace - [ ] Pending
...
- **Acceptance Criteria**:
  - [ ] `phosphor-icons/swift` is added as a pinned (`upToNextMajorVersion` from `2.0.0`) remote SPM dependency; project builds with no errors or warnings.
  - [ ] `SpreadTheme.Icon` exists with a case for every distinct icon currently in use, each resolving to the correct Phosphor `Image` + weight (`.regular` for outlines, `.fill` for `*.fill` SF Symbols).
  - [ ] Every `Image(systemName:)`/`systemImage:` usage across the app (including `Spread/Debug/*`) is migrated to `SpreadTheme.Icon`, with no SF Symbol strings remaining outside the mapping itself.
  - [ ] `RootNavigationView`'s `TabView`, `SpreadHeaderView`, `Action`'s menu labels, and `DebugRepositoryListView` render their icons via the `Image`-based label-closure forms instead of `systemImage:`.
  - [ ] No visual regression in icon sizing/color/active-vs-inactive state at any migrated call site.
- **Tests**:
  - [ ] Unit tests confirming each `SpreadTheme.Icon` case resolves to the expected Phosphor `Ph` case + weight (mirrors `SpreadThemeTests.swift`'s font-mapping test style).
