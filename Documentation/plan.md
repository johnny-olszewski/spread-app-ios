...
- **Acceptance Criteria**:
  ...
  - [x] The card appears only on the most granular spread that is `bestSpread(for: today)` (day > narrowest multiday > month > year) and there is at least one overdue task; it renders nothing on any other spread that merely contains today, and nothing when there are no overdue tasks.
  ...
  - [x] Tapping the status icon on an overdue row never toggles status; it shows a confirmation alert (spread source) or informational alert (Inbox source).
  - [x] Tapping anywhere else on an overdue row navigates directly to the task's source spread (spread source) or shows an informational alert (Inbox source) — no confirmation needed for the direct-navigate case.
  - [x] `SpreadsTabView`'s alert presentation is re-enabled and renders `spreadsCoordinator.activeAlert` correctly (verifiable via the pre-existing delete-entry/discard-changes alerts now actually appearing, not just the new overdue alerts).
...
- **Tests**:
  ...
  - [x] Unit test confirming the overdue row's configuration is read-only (no `onTitleCommit`, empty `actions`, both `onRowTap`/`onStatusIconTap` set).
  - [x] Unit test confirming a row tap on a spread-sourced task navigates directly (`coordinator.selectedSpread` updates, no alert).
  - [x] Unit test confirming a status-icon tap on a spread-sourced task shows the confirmation alert instead of navigating or toggling status.
  ...
  - [ ] Manual/visual verification: spot-check Month, Year, and Multiday spreads in the simulator with overdue tasks present, confirming card placement and chrome match Day's existing card, Month's calendar grid stays pinned while scrolling, the new tap/alert/navigate behavior works end-to-end (including confirming the now-re-enabled delete/discard-changes alerts elsewhere in the app still present correctly), and the card appears on only one spread at a time when nested spreads exist for today.
- **Progress (commits landed on feature/SESH-26)**:
...
- Remaining for this task: manual/visual verification across all 4 views, including the new tap/alert/navigate behavior, the granularity-priority fix, and the re-enabled alert system elsewhere in the app (deferred to the user — see Tests above).
