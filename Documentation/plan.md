...
- **AC correction (found during implementation)**: `SlashShape.path(in:)` always draws within the central 60% of its frame (20%–80% on each axis), so for the line to clear the base circle (diameter `s`, centered in the overlay frame) on both ends, the frame must exceed `s / 0.6 ≈ 1.667 * s`. Landed on `s * 2.2` — comfortably past that threshold. `strokeStyle.lineWidth` was deliberately left unchanged (still `max(1.5, s * 0.13)`, scaling off `s`): checked `.xmark`/`.arrowRight` and both already scale their line width off the icon's logical size `s`, not off their own (larger) decorative frame size — `.slash` now follows the same established convention rather than introducing a new one.
...
- **Acceptance Criteria**:
  - [x] `.slash`'s `frameSize` in `EntryStatusIcon.overlayView` is large enough that the diagonal line visibly extends past the base circle's edges on both ends, at the default 12pt icon size and at larger sizes used elsewhere (17/28/34pt, per the existing "Sizes" preview) — the `s * 2.2` multiplier scales with `s` at every size.
  - [x] `.slash`'s `strokeStyle.lineWidth` is unchanged, consistent with how `.xmark`/`.arrowRight` already scale off `s` rather than their own frame size (see AC correction above).
  ...
  - [x] No other `.slash` call site exists to re-verify — confirmed via `grep` that `EntryStatus.overlayShape`'s `.cancelled` branch is the only place `.slash` is referenced.
