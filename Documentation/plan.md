...
- **Deferred, not blocking task completion**: `MockChangeAwareTaskRepository`/`MockChangeAwareNoteRepository` (mirroring `MockTaskRepository`/`MockNoteRepository`) only if a SPRD-246/SPRD-248 test actually needs call-tracking or error injection rather than real in-memory persistence — confirm the need once SPRD-246 starts rather than building speculatively.
...
### [SPRD-246] Refactor: Additive JournalRuleEngine consolidating the pure logic seams - [ ] Open
...
- **Acceptance Criteria**:
  - [ ] New concrete (non-protocol, non-`any`) `JournalRuleEngine` type added, taking only `calendar`/`today`, covering data-model building, Inbox resolution, migration planning, overdue evaluation, and task/note assignment reconciliation.
  - [ ] `JournalRuleEngine` performs no repository writes; reconciliation methods only mutate the passed-in entity in place, consistent with existing reconciler behavior.
  - [ ] `MigratableEntry` and `EntryMigrationCandidate<E: AssignableEntry>` exist (carried forward from the removed first pass) and are used by the migration-planning methods.
...
- **Acceptance Criteria**:
  ...
  - [ ] New incremental index type added (e.g. `TaskIndex`/`NoteIndex` or a combined index) maintaining `SpreadDataModelKey ⇄ entity ID` mappings in both directions, updated incrementally per mutation rather than recomputed wholesale, with a single full-build path used only on cold load.
  - [ ] Resolving a single spread's `SpreadDataModel` (the `buildSpreadDataModel(for:)` replacement) costs O(entities indexed under that key) — implemented as an index bucket lookup plus dereference, with no `filter`/linear scan over the full task/note/event collections.
...
### [SPRD-252] Refactor: Split current assignment from migration history on Task/Note - [ ] Open

- **Context**: A SESH-24 analysis of `JournalDataModelAssembler`/`ConventionalJournalDataModelBuilder` found that every per-entity spread-association check (`hasSpreadAssociation`, `spreadKeys(for:)`) scans a task/note's *entire* `assignments` array, filtering out `.migrated` entries each time. That array is append-only in production — `StandardTaskMigrationCoordinator`, `StandardTaskAssignmentReconciler`, and `SpreadDeletionCoordinator` only ever `.append(...)` to it, never prune — so a task migrated repeatedly over months carries its full migration history (M entries) into every one of these checks, even though at most a handful of assignments are ever "live" at once. This compounds SPRD-247's index work: even with an O(1) index bucket lookup, computing which buckets an entity belongs to (`spreadKeys(for:)`) still costs O(M) per entity, not O(live assignments).
...
- **Acceptance Criteria**:
  - [ ] `Task`/`Note` expose a bounded "current assignments" collection separate from an append-only `migrationHistory` collection; spread-association logic (`spreadKeys(for:)`, the SPRD-247 index-maintenance path, overdue evaluation, migration planning) reads only the current collection.
  - [ ] Migration-history UI (wherever it reads `.assignments` today for display) is updated to read `migrationHistory` and is unaffected visually.
  - [ ] No change to the Supabase wire format/schema unless investigation during implementation proves one is actually required.
  - [ ] Project builds with no errors or warnings; full test suite passes.
- **Tests**:
  - [ ] Unit test proving a task with a large migration history (e.g. 100+ historical assignments) resolves `spreadKeys(for:)` in time independent of history length — via an entry-count instrumentation or equivalent, not a wall-clock benchmark.
  - [ ] Unit tests proving migration-history display content is unchanged for tasks/notes with existing history.
  - [ ] Regression coverage proving migration, reconciliation, and overdue evaluation behavior is unchanged for tasks/notes with both current and historical assignments.
- **Dependencies**: SPRD-249. Deliberately sequenced after cutover, not during the SPRD-245–248 additive phase: `Task`/`Note` are shared, non-isolated model types that both the legacy `JournalManager` stack and the new facade depend on simultaneously during that phase, so a model-shape change here cannot be additive-only (it would require editing legacy `Standard*` logic types too, contradicting SPRD-245–248's "zero edits to existing production files" rule). After SPRD-249 deletes the legacy stack, only one logic layer needs updating. Also benefits from pairing with SPRD-247's index work landing first, so the new "current assignment" shape can be threaded through index maintenance from a clean slate rather than retrofitted.

---

### [SPRD-253] Refactor: Additive TaskCoordinator/NoteCoordinator consolidating mutation + migration coordination - [ ] Open

- **Context**: Same root cause as SPRD-246's revised scope, applied to the write-side seams: `TaskMutationCoordinator` and `TaskMigrationCoordinator` both depend on exactly `taskRepository` (+ `calendar`) and nothing else — there is no reason "create/update/delete a task" and "migrate a task" are different protocol-backed types. Same for `NoteMutationCoordinator`/`NoteMigrationCoordinator` on the note side. Splitting these by concern name rather than by actual dependency shape is the same fragmentation problem SPRD-246 addresses for the pure-logic seams, just on the repository-writing side.
- **Description**: Add a concrete `TaskCoordinator` (no protocol, no "Standard" prefix) consolidating `StandardTaskMutationCoordinator` + `StandardTaskMigrationCoordinator`'s responsibilities: task creation, title/status/date-period/metadata updates, preferred-assignment clearing, deletion, single and batch migration. Depends on `TaskRepository` (protocol — the genuine substitution boundary, per SPRD-244's repository decision) and `JournalRuleEngine` (SPRD-246, for assignment reconciliation during create/update). Add the equivalent `NoteCoordinator` on the note side, depending on `NoteRepository` + `JournalRuleEngine`. Unlike `JournalRuleEngine`, these types do perform repository writes — that is their entire purpose, per the spec's distinction between rule engines (pure) and workflow coordinators (repository effects). Entirely new files; zero edits to the existing `StandardTaskMutationCoordinator`, `StandardTaskMigrationCoordinator`, `StandardNoteMutationCoordinator`, `StandardNoteMigrationCoordinator`, or their protocol declarations.
...
- **Acceptance Criteria**:
  - [ ] New concrete `TaskCoordinator` added, covering all `StandardTaskMutationCoordinator` + `StandardTaskMigrationCoordinator` responsibilities, depending only on `TaskRepository` and `JournalRuleEngine`.
  - [ ] New concrete `NoteCoordinator` added, covering all `StandardNoteMutationCoordinator` + `StandardNoteMigrationCoordinator` responsibilities, depending only on `NoteRepository` and `JournalRuleEngine`.
  - [ ] Neither type declares or conforms to a new protocol; no "Standard" naming.
  - [ ] No edits to any existing legacy mutation/migration coordinator file or protocol declaration.
  - [ ] Project builds with no errors or warnings.
- **Tests**:
  - [ ] Exhaustive unit tests for `TaskCoordinator`/`NoteCoordinator` covering the same scenarios as `EntryMutationCoordinatorTests`/`EntryMigrationCoordinatorTests`, constructing the type directly with a `Test*Repository` and a `JournalRuleEngine`, plus a parity test per consolidated method against its legacy `Standard*` counterpart.
- **Dependencies**: SPRD-245 (repository layer), SPRD-246 (`JournalRuleEngine`).

---

### [SPRD-254] Refactor: Additive concrete SpreadDeletionCoordinator - [ ] Open

- **Context**: Unlike the mutation/migration coordinators (SPRD-253), `SpreadDeletionCoordinator` has a genuinely distinct dependency shape — it touches `spreadRepository`, `taskRepository`, and `noteRepository` simultaneously — and a distinct, infrequent lifecycle (spread deletion, not routine entry mutation). Per CLAUDE.md's own coordinator-extraction guidance ("distinct lifecycle, distinct dependencies"), this earns staying its own type rather than merging into SPRD-253's `TaskCoordinator`/`NoteCoordinator`. It still needs the same "drop protocol, no Standard prefix" treatment as every other SPRD-246-family seam.
- **Description**: Add a concrete `SpreadDeletionCoordinator` (no protocol, no "Standard" prefix) consolidating `StandardSpreadDeletionCoordinator`'s and `StandardSpreadDeletionPlanner`'s responsibilities — planning which entries reassign to a parent spread vs. Inbox, then persisting those reassignments and the spread deletion. Depends on `SpreadRepository`/`TaskRepository`/`NoteRepository` (protocols) and `JournalRuleEngine` (SPRD-246) if assignment reconciliation is needed during reassignment. Entirely new file; zero edits to the existing `StandardSpreadDeletionCoordinator`, `StandardSpreadDeletionPlanner`, or their protocol declarations.
- **Spec**: `Documentation/Specs/JournalManager.md` — "Decision: Drop protocol-per-logic-seam; protocols are a repository-only boundary"
- **Acceptance Criteria**:
