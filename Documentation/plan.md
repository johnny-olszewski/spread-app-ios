...
- **Tests**:
  ...
  - [ ] Manual/visual verification: spot-check Month, Year, and Multiday spreads in the simulator with overdue tasks present, confirming card placement and chrome match Day's existing card, and confirming Month's calendar grid stays pinned (does not scroll) while the entry content beneath it scrolls.
- **Progress (commits landed on feature/SESH-26)**:
...
- Remaining for this task: manual/visual verification across all 4 views (deferred to the user — see Tests above).
