...
- **Progress (commits landed on feature/SESH-26)**
  ...
  - Remaining for this task: add the 8 `merge_X_batch` SQL functions to `baseline_schema.sql`; add `SyncSerializer.buildBatchMergeParams`; rework `SyncEngine.push()` to batch per entity type with updated `MergeRPCCaller` signature and partial-failure handling; remove unused scalar `merge_X` functions; apply schema to spread-prod (spread-dev skipped — decommissioned) with row-count/smoke-query verification.