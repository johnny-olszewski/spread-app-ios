...
- **Acceptance Criteria**:
  - [ ] Repositories and `SyncSerializer` read/write the new `entries`/`assignments`/`entry_tags` tables exclusively.
  - [ ] `Entry.date: Date?` exists on the base protocol; `DataModel.Event.date` returns `startDate`.
...
- **Progress (commits landed on feature/SESH-24)**:
  ...
  - Remaining for this task: hoist `Entry.date: Date?` (now bundled with the widen-types commit below); widen `Task.date`/`Task.period`/`Note.date` to optional; remove `hasPreferredAssignment`; rework the Task creation/edit UI toggle; collapse `SyncEntityType`; rewrite `SyncSerializer` and `SyncEngine`'s pull/apply dispatch for the new unified tables; apply the SQL migration to `spread-prod`; full test suite + build verification.

---

### [SPRD-248] Refactor: Additive JournalRuleEngine consolidating the pure logic seams - [ ] Open
...
### [SPRD-249] Refactor: Additive incremental dictionary-keyed index and JournalManager-equivalent facade - [ ] Open
...
### [SPRD-251] Refactor: Cut over to the new facade and delete the legacy JournalManager stack - [ ] Open
...
### [SPRD-252] Refactor: SerializableData protocol for sync entity serialization and entity typing - [ ] Open
...
### [SPRD-253] Refactor: Coalesce pending outbox mutations per entity - [ ] Open
...
### [SPRD-254] Refactor: Split current assignment from migration history on Task/Note - [ ] Open
...
- **Acceptance Criteria**:
  - [ ] `Task`/`Note` expose a bounded "current assignments" collection separate from an append-only `migrationHistory` collection; spread-association logic (`spreadKeys(for:)`, the SPRD-249 index-maintenance path, overdue evaluation, migration planning) reads only the current collection.
