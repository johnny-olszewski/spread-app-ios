...
- **Acceptance Criteria**:
  ...
  - [ ] Year and Multiday each show the same card chrome, grouping, and source chips as Day when applicable.
  ...
  - [ ] Multiday's existing per-day-card `overdueCount` badge mechanism (`MultidayDayCardView`) is untouched and unaffected.
...
- Remaining for this task: wire `OverdueCardView` into `YearSpreadContentView` and `MultidaySpreadContentView`; restructure `MonthSpreadContentView` to pin its calendar grid and wire `OverdueCardView` into its scrollable region; manual visual verification.
