...
### [SPRD-251] Refactor: Cut over to the new facade and delete the legacy JournalManager stack - [ ] In Progress
...
- Remaining for this task: rename `JournalDataStore` → `JournalManager` after deleting the legacy file, wire `DependencyContainer`/`AppRuntimeFactory`, fix any view call sites the rename doesn't cover automatically, delete legacy `Standard*` types/protocols, delete the legacy repository diffing path + apply SPRD-245's renames, port/delete legacy `JournalManager`-targeted tests, final full build/test/manual verification.
