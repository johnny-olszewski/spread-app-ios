...
### [SPRD-256] Refactor: Additive concrete SpreadDeletionCoordinator - [ ] In Progress
...
- **Acceptance Criteria**:
  ...
  - [x] Does not declare or conform to a new protocol; no "Standard" naming.
  - [ ] `JournalManager.deleteSpread`'s public signature is unchanged; its body becomes a thin delegation to the new coordinator plus the `upsertTask`/`upsertNote`/`removeSpread` patching the coordinator itself doesn't do.
...
- **Tests**:
  ...
  - [ ] Confirm `JournalManager`'s own existing test suites (`SpreadDeletionTests`, etc.) still pass unmodified after the delegation.
...
- Remaining for this task: wire `JournalManager.deleteSpread` to delegate to the new coordinator (replacing its inline body and removing the now-moved `replacementSpread`/`findParentSpread` private helpers), and confirm `SpreadDeletionTests.swift` passes unmodified.
