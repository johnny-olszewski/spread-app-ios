...
### [SPRD-268] Feature: Standardize SpreadTheme typography on Apple's 11-style scale + Mulish headings - [ ] Pending
...
- **Acceptance Criteria**:
  - `SpreadTheme.Typography` has all 11 of Apple's Dynamic Type styles, including a newly-added `headline`.
  - `title`/`title2`/`title3` and `heading(size:weight:)` use the bundled Mulish font instead of Avenir Next; Mulish font files + OFL license added under `Spread/Resources/Fonts/`, registered in `Info.plist`'s `UIAppFonts`, following the same pattern SPRD-267 established for Fuzzy Bubbles (separate static weight files, explicit PostScript-name mapping per `Font.Weight` — not `.weight()` chaining).
  - `largeTitle` remains on Fuzzy Bubbles (SPRD-267) — unchanged.
  - `body`/`callout`/`subheadline`/`footnote`/`caption`/`caption2` remain on system Dynamic Type styles — unchanged.
  ...
  - Project builds with no errors or warnings.
- **Tests**:
  - Unit tests confirming each bundled Mulish weight's PostScript name resolves via `UIFont(name:size:)` — mirrors `SpreadThemeTests.swift`'s existing Fuzzy Bubbles registration tests.
  - Unit test(s) confirming `heading(size:weight:)` resolves to the correct Mulish PostScript name per requested weight, including the fallback behavior for weights without a bundled file.
