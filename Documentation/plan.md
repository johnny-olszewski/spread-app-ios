...
### [SPRD-276] Refactor: Batch SyncEngine.push() mutations per entity type - [ ] Pending
...
- **Acceptance Criteria**:
  ...
  - [ ] Unused scalar `merge_X` functions are removed from `baseline_schema.sql` after confirming (by searching the codebase and any other known callers) that nothing else still calls them; any function found to still have a caller is kept and that caller is noted in the commit/progress notes.
  - [ ] The updated `baseline_schema.sql` is applied to `spread-prod` (spread-dev skipped — decommissioned per current account state), verified with `list_tables`/`execute_sql` smoke queries before/after — per the remote-migration-caution memory, confirm real row counts before applying.
...
- **Tests**:
  ...
  - [ ] Manual: queue edits across several entries/tasks while offline (airplane mode), go online, confirm via `mcp__supabase__get_logs` that exactly one RPC call per entity type fires and all entities land correctly server-side.
- **Progress (commits landed on feature/SESH-26)**
  ...
  - Remaining for this task: remove unused scalar `merge_X` functions from `baseline_schema.sql`; apply schema to spread-prod (spread-dev skipped — decommissioned) with row-count/smoke-query verification; manual offline/online airplane-mode test.