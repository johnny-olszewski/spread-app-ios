...
- Remaining for this task: bidirectional `SpreadDataModelKey ⇄ entity ID` index for tasks/notes (`[2/n]`), event indexing (`[3/n]`), the facade type wiring everything together (`[4/n]`+), and the four remaining test categories (parity vs. full rebuild, single-mutation-no-full-rebuild, O(1)-lookup-under-large-N, cold-load-exactly-once).
