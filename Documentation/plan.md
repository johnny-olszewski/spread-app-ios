...
- **Acceptance Criteria**:
  ...
  - [x] Project builds with no errors or warnings.
- **Tests**:
  - [ ] Manual verification per spread: toggling group-by/order-by re-partitions/re-orders correctly; persistence is independent per spread type. **Not yet performed for Month/Year/MonthCard/Multiday** — only the Year scroll fix was attempted (unreliably) in the simulator this session; recommend a manual pass across all four before merging.
...
- Remaining for this task: manual simulator verification across Month/Year/MonthCard/Multiday (Tests AC).
