...
extension YearSpreadContentView {
    ...
    /// Owns the entry row configuration map for `YearSpreadContentView`.
    ///
    /// Decouples the view from configuration-map construction logic. Coordinator-dependent
    /// actions (edit task / note) are injected as closures so the ViewModel stays free of
