...
struct JournalManagerParentSpreadsTests {
    ...
    /// Conditions: Current spread is a year spread. Year spreads have no broader-period parent.
    /// Expected: Always returns [].
    @Test @MainActor func yearPeriodReturnsEmpty() async throws {
    ...
    /// Conditions: Today is July 6, 2026. No year spread exists. Viewing a month spread.
    /// Expected: Returns [] (no "This Year" shortcut when the spread doesn't exist).
    @Test @MainActor func monthPeriodReturnsEmptyWhenNoYearSpreadExists() async throws {
        let calendar = Self.testCalendar
        let today = Self.date(year: 2026, month: 7, day: 6)
        let monthSpread = DataModel.Spread(period: .month, date: today, calendar: calendar)
    ...
    /// Expected: Returns [monthSpread, yearSpread] — same as day period, always anchored to today.
